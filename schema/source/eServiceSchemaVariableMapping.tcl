...
###############################################################################
#                                                                             #
#   Copyright (c) 1998-2018 Dassault Systemes.  All Rights Reserved.                 #
...
proc eServiceInstallAdmin { lCmdList ItemType } {
    ...
    set prefixName    "eService${sVersion}_"
...
proc eServiceAddAdminObj {sOperation fileId} {
    ...
    if {"$outStr" == "TRUE"} {
        ...
        if {"$fileId" != ""} {
            set sMQLCmdNew [regsub -all "\\\\\"" $sMQLCmd "QQQ"]
            set sMQLCmdNew [regsub -all "\"" $sMQLCmdNew "\\\""]
            set sMQLCmdNew [regsub -all "QQQ" $sMQLCmdNew "\\\\\\\\\\\""]
            set sMQLCmdNew [regsub "mql" $sMQLCmdNew ""]
            set sMQLCmdNew "${sMQLCmdNew};"
            set sMQLCmdNew "puts $fileId \"${sMQLCmdNew}\""
            eval {uplevel  "$sMQLCmdNew"}
        }
...
proc eServiceDeleteAdminObj {sOperation fileId} {
...
proc eServiceModifyAdminObj {sOperation fileId} {
...
proc eServiceConnectAdminObj {sOperation fileId} {
    set sInstallDir [mql get env MXAPPBUILD]
    set lVersions [mql get env MXVERSIONLISTTOINSTALL]
    set sVersion [lindex $lVersions 0]
...
proc eServiceDisconnectAdminObj {sOperation fileId} {
...
proc eServiceModifyAdminObjConnection {sOperation fileId} {
...
proc eServiceImmutableAdminObj {sOperation fileId} {
    set sInstallDir [mql get env MXAPPBUILD]
    set lVersions [mql get env MXVERSIONLISTTOINSTALL]
    set sVersion [lindex $lVersions 0]
    ...
    upvar         sName sName
    set sRev      [lindex $sOperation 4]
    set sImmutable    [lindex $sOperation 5]
    upvar         sMode sMode
    ...
    puts stdout [format ">Immutable  :%-13s" $isLock]

    if {[info exists ::env(LOG_MQL)] && "$::env(LOG_MQL)" == "TRUE"} {
        puts "mql exec program com.matrixone.apps.domain.util.ImmutableUtil -method emxSetImmutable \"$isLock\" object \"$sAbsType\" \"$sName\" \"$sRev\""
    }
    if {"$fileId" != ""} {
        puts $fileId " exec program com.matrixone.apps.domain.util.ImmutableUtil -method emxSetImmutable \"$isLock\" object \"$sAbsType\" \"$sName\" \"$sRev\";"
    }

    set sCmd {mql exec program com.matrixone.apps.domain.util.ImmutableUtil -method emxSetImmutable "$isLock" object "$sAbsType" "$sName" "$sRev"}
    set mqlret [catch {eval $sCmd} outStr]
    if {$mqlret != 0} {
        puts stdout ">ERROR       : eServiceImmutableAdminObj"
        puts stdout ">$sCmd"
        puts stdout ">$outStr"
        return 1
    }
...
proc eServiceChangeAdminObjState {sOperation fileId} {
    ...
    if {"$sCurrentState" == "$sTargetState"} {
        ...
        for {set i 0} {$i < $sDiff} {incr i} {
            ...
            if {"$fileId" != ""} {
                puts $fileId " override bus \"$sAbsType\" \"$sName\" \"$sRev\";"
            }
            ...
            if {"$fileId" != ""} {
                puts $fileId " $sAction bus \"$sAbsType\" \"$sName\" \"$sRev\";"
            }
...
proc eServiceInstallAdminBusObjs {lOperations} {
    ...
    if {[info exists ::env(DS_IN_CLOUD)] == 1 && [file isdirectory [file join "$sInstallDir" "AppInstall" "Programs"]] == 1} {
        set fileId [open [file join "$sInstallDir" "AppInstall" "Programs" "AdminBusObj${sVersion}.tcl"] a+]
    }
    ...
    foreach sOperation $lOperations {
        ...
        switch $sFunction {
            "ADD_OBJECT"
                {
                    set mqlret [eServiceAddAdminObj "$sOperation" "$fileId"]
                }
            "MODIFY_OBJECT"
                {
                    set mqlret [eServiceModifyAdminObj "$sOperation"  "$fileId"]
                }
            "DELETE_OBJECT"
                {
                    set mqlret [eServiceDeleteAdminObj "$sOperation" "$fileId"]
                }
            "CONNECT_OBJECTS"
                {
                    set mqlret [eServiceConnectAdminObj "$sOperation" "$fileId"]
                }
            "DISCONNECT_OBJECTS"
                {
                    set mqlret [eServiceDisconnectAdminObj "$sOperation" "$fileId"]
                }
            "MODIFY_CONNECTION"
                {
                    set mqlret [eServiceModifyAdminObjConnection "$sOperation" "$fileId"]
                }
            "CHANGE_STATE"
                {
                    set mqlret [eServiceChangeAdminObjState "$sOperation" "$fileId"]
                }
            "IMMUTABLE"
                {
                    set mqlret [eServiceImmutableAdminObj "$sOperation" "$fileId"]
                }
            default
                {
    ...
    if {[info exists ::env(DS_IN_CLOUD)] == 1 && "$fileId" != ""} {
        close $fileId
    }
...
proc emxCreateMQLForMenu {lMenuInfo sMenuName bMenuInstalled sOperation} {
    ...
    set sRegProgName [mql get env REGISTRATIONOBJECT]
    set sInstallVersion [mql get env MXVERSION]
    set sPrefixName "emx${sInstallVersion}_"
    set sPixDir [mql get env MXPIXMAPDIR]
                    ...
                    switch -regexp $sKey {
                        ...
                        "command" {
                            ...
                            foreach sCommand $lCommands sOrder $lOrders {
                                ...
                                set afterCmd [lindex $lExistingCmd [expr $sOrder - 1]]
                                
                                if {"$afterCmd" != "$sCommand"} {
...
proc emxInstallProgram {lProgramList} {
    ...
    foreach lProgramInfo $lProgramList {
        ...
        set nCodeIndex [lsearch $lKeys "code"]
        if {"$nCodeIndex" == "-1"} {
        ...
        if {$nCodeIndex == -1 && $nBaseIndex == -1 && $nFileIndex == -1} {
        ...
        if {"$sProgramName" == "$outStr"} {
            ...
            if {"$nCodeIndex" >= 0 || ("$nForceIndex" >= 0 && "$sAccessVal" != "$sProgAcess")} {
                ...
                if {"$sAccessVal" == "READ_WRITE"} {
                    ...
                    foreach sKey $lKeys sValue $lValues {
                        switch $sKey {
                            user {
                                set userName [eServiceGetCurrentSchemaName person eServiceSchemaVariableMapping.tcl $sValue]
                                append sCmd " execute $sKey '$userName'"
                            }
                            description {
                                append sCmd " $sKey '$sValue'"
                            }
                ...
                # Else if Access = READ_WRITE then
                    ...
                    # unlock program before modifying
                    if {"$sProgramOrPage" == "program"} {
                        set progImmutable [mql print program "$sProgramName" select immutable dump]
                        if {"$progImmutable" == "TRUE"} {
	                    set sImmutableCmd "mql exec program com.matrixone.apps.domain.util.ImmutableUtil -method emxSetImmutable unlock '$sProgramOrPage' '$sProgramName'"
                 	    set mqlret [ catch {eval $sImmutableCmd} outStr ]
	                    if {$mqlret != 0} {
		                puts stdout ">ERROR       : emxInstallProgram"
		                puts stdout ">$outStr"
		                set mqlall 1
                    ...
                    if {"$sProgramOrPage" == "program"} {
                        if {"$progImmutable" == "TRUE"} {
	                    set sImmutableCmd "mql exec program com.matrixone.apps.domain.util.ImmutableUtil -method emxSetImmutable lock '$sProgramOrPage' '$sProgramName'"
                 	    set mqlret [ catch {eval $sImmutableCmd} outStr ]
	                    if {$mqlret != 0} {
		                puts stdout ">ERROR       : emxInstallProgram"
		                puts stdout ">$outStr"
        ...
        set nImmutableIndex [lsearch $lKeys "immutable"]
        set sImmutableValue ""
        if {$nImmutableIndex > -1} {
            set sImmutableValue [lindex $lValues $nImmutableIndex]
        }

	if {"$sImmutableValue" == "true"} {
	    set sCmd "mql exec program com.matrixone.apps.domain.util.ImmutableUtil -method emxSetImmutable lock '$sProgramOrPage' '$sProgramName'"
	...
	} elseif {"$sImmutableValue" == "false"} {
	    set sCmd {mql exec program com.matrixone.apps.domain.util.ImmutableUtil -method emxSetImmutable unlock "$sProgramOrPage" "$sProgramName"}
	    set mqlret [ catch {eval $sCmd} outStr ]
	    if {$mqlret != 0} {
		puts stdout ">ERROR       : emxInstallProgram"
		puts stdout ">$outStr"
...
proc emxMakeTriggersImmutable {lOperations} {
    ...
    puts "$sOperation"
        set sComment [lindex $sOperation 0]
        set sAdmin [lindex [split [lindex $sOperation 1] "_"] 0]
        if {"policy" == "$sAdmin"} {
            set sAdminName [emxGetCurrentSchemaName $sAdmin "eServiceSchemaVariableMapping.tcl" "[lindex [split [lindex $sOperation 1] :] 0]"]
            puts "sAdminName = $sAdminName"
            set sStateName [emxGetCurrentSchemaName "state" "eServiceSchemaVariableMapping.tcl" "[lindex [split [lindex $sOperation 1] :] 0]" "[lindex [split [lindex $sOperation 1] :] 1]"]
                        puts "sStateName = $sStateName"

        } else {
            set sAdminName [emxGetCurrentSchemaName $sAdmin "eServiceSchemaVariableMapping.tcl" "[lindex $sOperation 1]"]
            set sStateName ""
        }
        set sEvent [lindex $sOperation 2]
        set sType [lindex $sOperation 3]
        set sLock [lindex $sOperation 4]
        puts "$sComment"
        if {"$sLock" == "true"} {
            set isLock "lock"
        } else {
        ...
        set sCmd {mql exec program com.matrixone.apps.domain.util.ImmutableUtil -method emxSetImmutable "$isLock" "$sAdmin" "$sAdminName" "$sEvent" "$sType" "$sStateName"}
        set mqlret [ catch {eval $sCmd} outStr ]
        if {$mqlret != 0} {
            puts stdout ">ERROR       : emxMakeTriggersImmutable"
            puts stdout ">$outStr"
            return 1
        }
    }

	...
	proc createFileVersionObject {oID sFormat sNewFilepath {sLatestVersion ""} {sVersionPrefix ""} } {
		...
		if {[lsearch $::VERSIONABLETYPEINCLUSIONLIST "$sType"] >= 0} {
			...
			if {[::xBO::getBOExist "$sType" "$sName" "$sRev"] == $::TRUE} {
				...
				set xObjCreate(0) $::TRUE
				set xObjCreate(OID) [::xBO::getBOID "$sType" "$sName" "$sRev"]
			} else {
			...
			set x [::xBO::setBOAttributeValue $oVerID "Is Version Object" "True"]
				if {$x != $::TRUE} {
					set bcreateFileVersionObject [::xUtil::errMsg 48 "could not update Version attribute(Is Version Object): $sType $sName $sRev Is Version Object =yes"]
				}

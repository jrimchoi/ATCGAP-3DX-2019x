...
#
# $RCSfile: DECCommonNumberGenerator.tcl.rca $ $Revision: 1.50 $
#
# @progdoc      DECCommonNumberGenerator.tcl
#
...
#
#
# @progdoc      Copyright (c) 1993-2011, Dassault Systemes. All Rights Reserved.
...
proc eServiceNumberGeneratorNEW { ObjectGeneratorName ObjectGeneratorRevision ObjectPolicy {CreateAdditional Null} {ObjectVault ""} {CustomRevisionLevel ""} {ObjectType ""} {UniqueNameOnly "No"} {UseSuperUser "Yes"}} {

    eval  [utLoad eServiceSchemaVariableMapping.tcl]
	
    # eServicecommonShadowAgent is commented as per directions from BPS, for addressing security issue 
	...
	#check if only name is needed (not object)
	if {[string match "Yes*" "$UniqueNameOnly"] == 1 } {
	    #only name needed, check if count is passed
	    set iCount [string trim [lindex [split $UniqueNameOnly |] 1]]
	    if {[string trim $iCount] == "" } {
		#count not passed; only _*one*_ name needed
		set iCount 1
	    }
	} else {
	    #both name and object needed;so only _*one*_ name supported, count is ignored
	    set iCount 1
	}
	...
	set bContinue true
        ...
        while {$bContinue} {
            ...
            # Following code is commented as per directions from BPS, for addressing security issue
            # pushShadowAgent
		...
		set sModNextNumber $sNextNumber
		
		for {set j 0} {$j<$iCount} {incr j} {

		    # Generate the new name for the primary object
		    set sNewName "$sNamePrefix$sModNextNumber$sNameSuffix"
		    lappend lNewNames $sNewName

		    if {$j<$iCount} {
			set sModNextNumber [utStrSequence $sModNextNumber 1]
		    }
		}
		...
		if { $CustomRevisionLevel == "" } {                    
		    ...
		    set sRev "$CustomRevisionLevel"
                ...
                for {set k 0} {$k < $iCount} {incr k} {

		    set sNewName [lindex $lNewNames $k]
		    if {$mqlret == 0} {
			...
			if {$mqlret == 0} {
				...
				if {$sCurrent >= $sMaxProcessTime} {
				    ...
				    #object exists, but time limit not reached;break from the for loop, but not from the autoname generation while loop.
				    #we need to re-start name generation for the entire count
				    break
				}
			    } else {
				# object does not exist, 
				# leave the autoname generation loop, if count is reached
				if {$k == [expr {$iCount-1}]} {
        ...
        if {$mqlret == 0} {
            ...
            if {[string match "Yes*" "$UniqueNameOnly"] == 1 } {
		set lConSolidateSet [concat $lConSolidateSet $lNewNames]
            } else {
    ...
    if {$mqlret == 0} {
     ...
     if {$mqlret == 0} {
      ...
      set sNewInfo [eServiceNumberGeneratorNEW "$sObjGenName" "$sObjGenRev" "$sPolicy" "$sCreateAdditonal" "$sVault" "$sCustomRevLevel" "$sObjectType" "$sUniqueNameOnly" "$sUseSuperUser"]
	    ...
	    if {[string match "Yes*" "$sUniqueNameOnly"] == 1 } {
		append lResult "0"
		foreach i $sNewInfo {
		
		    append lResult "|[string trim [lindex "$i" 0]]"
		}
           } else {
              foreach i $sNewInfo {

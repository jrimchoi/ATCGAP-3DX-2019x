...
#
# $RCSfile: emxParameterNumberGenerator.tcl.rca $ $Revision: 1.50 $
#
# @progdoc      emxParameterNumberGenerator.tcl
#
...
#
#
# @progdoc      Copyright (c) 1993-2017, Dassault Systemes. All Rights Reserved.
...
###############################################################################
#                                                                             #
#   Copyright (c) 1998-2017 Dassault Systemes.  All Rights Reserved.                 #
...
proc eServiceComposeeNumberGenerator { ObjectGeneratorName ObjectGeneratorRevision ObjectPolicy {CreateAdditional Null} {ObjectVault ""} {CustomRevisionLevel ""} {ObjectType ""} {UniqueNameOnly "No"} {UseSuperUser "Yes"} {Relation ""} {ComposerT ""} {ComposerN ""} {ComposerR ""}} {
	...
	set Relation				 [string trim $Relation]
	set ComposerT				 [string trim $ComposerT]
	set ComposerN				 [string trim $ComposerN]
	set ComposerR				 [string trim $ComposerR]

    # set program related variables
    set progname      "emxParameterNumberGenerator.tcl"
        ...
        if {$mqlret == 0} {
            ...
            if {"$UniqueNameOnly" == "Yes"} {
                ...
                if {$sVault == "ADMINISTRATION"} {
					...
					if {("$Relation" == "") || ("$ComposerT" == "") || ("$ComposerN" == "") || ("$ComposerR" == "")} {
						...
						set sCmd {mql add bus $sPrimaryObjectType $sNewName $sRev composer "$ComposerT" "$ComposerN" "$ComposerR" composition "$Relation" policy "$ObjectPolicy" select id to\[$Relation\].id dump}
						set sReturnVault $sVault
					}
                }
                set mqlret [catch {eval $sCmd} outstr]
				set sNewRelId ""
                 ...
                 if {$mqlret == 0} {
					set relcomp [string first "," "$outstr"]
					if {$relcomp == -1} {
						set sNewObjectID $outstr
					} else {
						set sNewObjectID [string range "$outstr" 0 $relcomp-1]
						set sNewRelId [string range "$outstr" $relcomp+1 [string length "$outstr"]]
					}
                 }
              ...
              #  }
                ...
                if {$mqlret == 0} {                   
					...
					if {"$sNewRelId" ne ""} {
						lappend lConSolidateSet "$sNewRelId"
					}
                }
    ...
    if {$mqlret == 0} {
     ...
     set lResult ""
     set sProgName "emxParameterNumberGenerator.tcl"
     set RegProgName   "eServiceSchemaVariableMapping.tcl"
     set mqlret 0
	 ...
	 set sRelation		  [mql get env 10]
	 set sComposerT		  [mql get env 11]
	 set sComposerN		  [mql get env 12]
	 set sComposerR		  [mql get env 13]

     #new number generator
     if {$mqlret == 0} {

      set sNewInfo [eServiceComposeeNumberGenerator "$sObjGenName" "$sObjGenRev" "$sPolicy" "$sCreateAdditonal" "$sVault" "$sCustomRevLevel" "$sObjectType" "$sUniqueNameOnly" "$sUseSuperUser" "$sRelation" "$sComposerT" "$sComposerN" "$sComposerR"]

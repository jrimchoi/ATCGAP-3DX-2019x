...
proc pushShadowAgent {{userAgent "person_UserAgent"}} {
	...
	#Retreive MX_LOGGED_IN_USER_NAME variable from environment
	set rpeUserName [mql get env global MX_LOGGED_IN_USER_NAME]
	#Set MX_LOGGED_IN_USER_NAME variable with logged in user
	if { $rpeUserName == "" || $rpeUserName == "null"} {
		set mqlret [catch {eval mql set env global MX_LOGGED_IN_USER_NAME \"$sUser\" } outStr ]
	}

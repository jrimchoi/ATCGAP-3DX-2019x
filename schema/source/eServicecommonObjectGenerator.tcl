...
proc eServiceNumberGenerator { ObjectGeneratorName ObjectGeneratorRevision ObjectPolicy {CreateAdditional Null} {ObjectVault ""} {CustomRevisionLevel ""} {ObjectType ""} {UniqueNameOnly "No"} {UseSuperUser "Yes"}} {
	...
	while {1} {
	    ...
	    set sCmd {mql exec program eServicecommonGetNextNumber.tcl "$sRetryCount" "$sRetryDelay" "$sNumberGeneratorId"}
	    set mqlret [catch {eval $sCmd} outstr]
	    # Compose the new name and verify its uniqueness
	    if {$mqlret != 0} {
		return "$outstr"
	    } else {
		set sNextNumber [lindex $outstr 0]
	...
	if {$mqlret == 0} {
		...
		if {"$UseSuperUser" != "No"} {
		    set sCmd {mql exec program eServicecommonGenerateObject.tcl "$sVault" "$sPrimaryObjectType" "$sNewName" "$sRev" "$sSafetyVault" "$ObjectPolicy" "$sRealUser" "$CreateAdditional" "$ObjectGeneratorName" "$ObjectGeneratorRevision"}
		    set mqlret [catch {eval $sCmd} outstr]
		    set lConSolidateSet "$outstr"
		} else {
		    # Add object

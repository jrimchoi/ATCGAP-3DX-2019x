...
<%
			...
			// ENGMASA : Modified below property to exclude CA owner or logged in user incase of creation from reviewers selection : START
			String strOwner = "";
			// ENGMASA : Modified below property to exclude CA owner or logged in user incase of creation from reviewers selection : END
			...
			if(!UIUtil.isNullOrEmpty(objectID)){
				...
				// ENGMASA : Modified below property to exclude CA owner or logged in user incase of creation from reviewers selection : START
				strOwner = dmObj.getInfo(context, DomainConstants.SELECT_OWNER);
				// ENGMASA : Modified below property to exclude CA owner or logged in user incase of creation from reviewers selection : END
...
function setOwningOrg()
{
	...
	//XSSOK
	document.forms['editDataForm'].elements["OwningOrganizationDisplay"].value		=	"<%=companyName%>";
}
...
function setConnectedOrg()
{
	...
	//XSSOK
	document.forms['editDataForm'].elements["OwningOrganizationDisplay"].value		=	"<%=sConnectedName%>";
}
...
function validateCancelCA() { 
	var reason    			= document.forms['editDataForm'].elements["Reason"].value;
    if(reason!="")  {
		   var confirmmessage="<emxUtil:i18nScript localize="i18nIdn">EnterpriseChangeMgt.CancelCADialog.CancelCAConfirm</emxUtil:i18nScript>"; 		   
           if (confirm(confirmmessage)){
               return true;        
            } else {
               return false;
...
function validateHoldCA() { 
	var reason    			= document.forms['editDataForm'].elements["Reason"].value;
	if(reason!="")  {
		   var confirmmessage="<emxUtil:i18nScript localize="i18nIdn">EnterpriseChangeMgt.HoldCA.ConfirmMsg</emxUtil:i18nScript>"; 		   
           if (confirm(confirmmessage)){
               return true;        
            } else {
               return false;
...
function addReviewSelectors(){
	...
	// ENGMASA : Modified below property to exclude CA owner or logged in user incase of creation from reviewers selection : START
	var sURL=	'../common/emxFullSearch.jsp?field=TYPES=type_Person:CURRENT=policy_Person.state_Active:NAME!=<%=strOwner%>&excludeOID='+ReviewersHidden.value+'&table=AEFGeneralSearchResults&selection=multiple&hideHeader=true&submitURL=../enterprisechangemgtapp/ECMUtil.jsp?mode=searchUtilReviewer&targetTag=select&selectName=Reviewers';
	// ENGMASA : Modified below property to exclude CA owner or logged in user incase of creation from reviewers selection : END

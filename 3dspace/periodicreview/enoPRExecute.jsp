...
<%	
	final String action = emxGetParameter(request, PeriodicReviewConstants.PR_ACTION);
	...
	try 
	{
		...
		{
           throw new Exception("Error: Mandatory parameter 'prAction' missing.");
        }
		...
		strJavaScript = (String)mResponse.get(PeriodicReviewConstants.ACTION_JAVASCRIPT);
		strInclude = (String)mResponse.get(PeriodicReviewConstants.ACTION_INCLUDEJSP);
		saIncludeJS = (String[])mResponse.get(PeriodicReviewConstants.ACTION_INCLUDEJAVASCRIPT);
		strAjax = (String)mResponse.get(PeriodicReviewConstants.ACTION_AJAX);
	}
	catch (Exception exp) {
		exp.printStackTrace();

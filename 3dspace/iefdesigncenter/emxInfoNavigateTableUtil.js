...
var IMG_LINE_VERT = DIR_TREE + "utilLineVert.gif";
var IMG_LINE_LAST = DIR_TREE + "utilLineLast.gif";
var IMG_LINE_LAST_OPEN = DIR_TREE + "utilLineLastOpen.gif";
var IMG_LINE_LAST_CLOSED = DIR_TREE + "utilLineLastClosed.gif";
var IMG_LINE_NODE = DIR_TREE + "utilLineNode.gif";
var IMG_LINE_NODE_OPEN = DIR_TREE + "utilLineNodeOpen.gif";
var IMG_LINE_NODE_CLOSED = DIR_TREE + "utilLineNodeClosed.gif";
...
function jsDocument()
{
	...
	this.write("<html><head><script language=\"JavaScript\" src=\"../integrations/scripts/MCADUtilMethods.js\" type=\"text/javascript\"></script><script language=\"JavaScript\" src=\"../common/scripts/emxUIConstants.js\" type=\"text/javascript\"></script></script><script language=\"JavaScript\" src=\"emxInfoUIModal.js\" type=\"text/javascript\"></script><script language=\"JavaScript\" src=\"../common/scripts/emxUICore.js\" type=\"text/javascript\"></script>");
	this.writeln("");
	if (isUnix)
	{
		this.writeln("<link rel=\"stylesheet\" type=\"text/css\" href=\"../common/styles/emxUIDefault_Unix.css\" />");
		this.writeln("<link rel=\"stylesheet\" type=\"text/css\" href=\"../common/styles/emxUIList_Unix.css\" />");
		this.writeln("<link rel=\"stylesheet\" type=\"text/css\" href=\"../common/styles/emxUIToolbar_Unix.css\" />");
		this.writeln("<link rel=\"stylesheet\" type=\"text/css\" href=\"../common/styles/emxUIMenu_Unix.css\" />");
	}
	else
    {
		this.writeln("<link rel=\"stylesheet\" type=\"text/css\" href=\"../common/styles/emxUIDefault.css\" />");
		this.writeln("<link rel=\"stylesheet\" type=\"text/css\" href=\"../common/styles/emxUIList.css\" />");
		this.writeln("<link rel=\"stylesheet\" type=\"text/css\" href=\"../common/styles/emxUIToolbar.css\" />");
		this.writeln("<link rel=\"stylesheet\" type=\"text/css\" href=\"../common/styles/emxUIMenu.css\" />");
	}
...
function jsTable(tablewidth,numOfCols) {
	...
	//frame name
	this.treeFrame = "listDisplay"; //tableDisplay";
	//width
...
function _jsTree_drawChild(d, node) {
	...
	if (node.selected == true){
	  d.write("<tr class=selected height="+varRowHeight+">");
	}else{
	  d.write("<tr height="+varRowHeight+">");
	}
	...
	if (children_found){
		if (varViewRelArrows){
			if (node.indent > 0) {
              ...
              d.write("<a href=javascript:parent.openModelessDialog(\"" + pageURL + "\") >"  + node.relImage + "&nbsp;</a>");
	...
	if (varViewIcons){
		...
		d.write("\" border=\"0\" width=\"16\" height=\"14\" align=absmiddle alt=\"" +  sIconToolTip + "\">&nbsp;");
	...
	if (children_found)
	{
		...
		{
			/*var pageURL ="";
		    if(varBusObjectDetailsPage.indexOf("?")==-1)
			pageURL = varBusObjectDetailsPage + "?treeMenu=" + node.menuName + "&objectId=" + node.id ;
			else
			pageURL = varBusObjectDetailsPage + "&treeMenu=" + node.menuName + "&objectId=" + node.id ;
			*/
            if(node.readAccess==true){
			var pageURL = varBusObjectDetailsPage +  "?objectId=" + node.id ;
		...
		{
			if(node.readAccess==true)
			 d.write("&nbsp;<a href=\"" + varBusObjectDetailsPage + "?objectId=" + node.id + "\"  class='"+node.cueClass+"' title='"+node.objTip+"' alt='"+node.objTip+"' target=content >");
		}
	}
	...
	//display the window icon to display object details in a pop up window
	//if (node.id ){
	//	var pageURL = varBusObjectDetailsPage + "?treeMenu=" + node.menuName + "&objectId=" + node.id;
	//	var iconUrl="";
	//	if(node.readAccess==true)
	//	iconUrl="&nbsp;<a href=javascript:parent.openModelessDialog(\"" + pageURL + "\")><img src=\"../common/images/iconActionNewWindow.gif\" border=\"0\" width=\"16\" height=\"14\" align=absmiddle/></a>";
	//	else
	//	iconUrl="&nbsp;<a><img src=\"../common/images/iconActionNewWindow.gif\" border=\"0\" width=\"16\" height=\"14\" align=absmiddle/></a>";
	//	d.write(iconUrl);
	//}

	d.writeln("</td></tr>");

	d.writeln("<tr><td class=line colspan=2><img src=images/utilSpace.gif width=1 height=1></td></tr>");
...
function _jsTree_drawMiscImages(d, node) {
	...
	while (i < node.indent - 1) {
		...
		tempstr += "\" width=\"19\" height=\"14\" border=\"0\"  align=absmiddle >&nbsp;"
...
function _jsTree_drawPlusMinusImage(d, node) {
	...
	d.write("\" border=\"0\" width=\"19\" height=\"14\" name=\"node");
	d.write(node.nodeID);
...
function _jsTree_drawLoadingMessage(d, node) {
	...
	d.write(IMG_LOADING);
	d.write("\" border=\"0\" width=\"16\" height=\"14\"></td><td nowrap class=\"loading\">");
	d.write(STR_LOADING);
	d.write("</td></tr></table>");
...
function _jsTree_setScrollPosition() {
	...
	frames['listDisplay'].scrollTo(this.scrollX, this.scrollY);
	return;
}
...
*/
	...
	d.writeln("<tr><td class=line colspan=" + (2*localTable.numOfCols + 1) + " ><img src=images/utilSpace.gif width=1 height=1></td></tr>");
...
function _jsTable_draw() {
	...
	if (this.root) 
	{
		...
		d.write("<form name=emxTableForm method=post>");
		
		d.write("<table cellpadding=0 cellspacing=0 border=0 >");
		d.write("<tr height="+varRowHeight+">" + this.heading + "</tr>");
		...
		d.write("<input type=\"hidden\" name=\"commandName\" value=\"Navigate\">");
		d.write("</form>");
		//write the footer
		d.write("</body></html>");
...
function scrollCheck() {
	if (document.all){
		when = setTimeout('scrollCheck()',500);
		if( frames['listDisplay'] && frames['folderDisplay'] && frames['folderDisplay'].document.body)
			frames['listDisplay'].scrollTo(frames['listDisplay'].document.body.scrollLeft,frames['folderDisplay'].document.body.scrollTop);
	}
	else{
		when = setTimeout('scrollCheck()',500);
		if( frames['listDisplay'] && frames['folderDisplay'] )
			frames['listDisplay'].scrollTo(frames['listDisplay'].pageXOffset,frames['folderDisplay'].pageYOffset);
	}
	return;
}

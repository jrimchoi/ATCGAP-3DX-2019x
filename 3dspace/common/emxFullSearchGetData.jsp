...
<%
	String redirectToSnN = Request.getParameter(request, "action");
		...
		if(searchNextGen.getRedirectToSnN() && (queryType == null || queryType.equals("Indexed"))){
				...
				/*String sJSONValue= refinementForSnN.toString();
				sJSONValue = sJSONValue.replace("\"default_search_criteria\":\"\"", "\"default_search_criteria\":\"entercriteria\"");					
				JsonReader jsonReader = Json.createReader(new StringReader(sJSONValue));
				refinementForSnN = jsonReader.readObject();
				jsonReader.close();*/

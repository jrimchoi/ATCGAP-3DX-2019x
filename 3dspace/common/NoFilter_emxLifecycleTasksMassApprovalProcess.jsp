...
<% 
    ...
    try {
		...
		if(fromPage!=null && fromPage.equals("APPUserTaskMassApproval")) {
			for (Iterator itrObjectList = objectList.iterator(); itrObjectList.hasNext();) {
				...
				mapCurrObject.put("Approval Status" + strSerialNumber,strApprovalStatusAction);
				mapCurrObject.put("Comments" + strSerialNumber,strComments);
			}
			tableDataMap.put("ObjectList",objectList);
			tableBean.setTableData(timeStamp,tableDataMap);
			
			String processRouteAssignee   = emxGetParameter(request,"processRouteAssignee");
			if("true".equals(processRouteAssignee) == false){
				%>
				<script language="Javascript" >
					emxShowModalDialog('../drV6Tools/drUpdateMyTasksMassApprovalRouteAssignees.jsp?timeStamp=<%=XSSUtil.encodeForURL(context, timeStamp)%>',575,575); 
				</script>
				<%return;
			}
		}

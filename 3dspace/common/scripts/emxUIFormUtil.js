...
// ENGMASA : Added for gapFrameModel ranges : START
function modifygapFrameModelSize(vModelSizes)
{
	var vSelectedRange = jQuery("#gapFrameModel").val();
	
	jQuery("#gapFrameModelSize").empty();
	var vVal = new Array();
	vVal = vModelSizes.split(",");
	var arrayLength = vVal.length;
    for (var i = 0; i < arrayLength; i++) {
		var vLine = vVal[i];
		var vRangeVal = new Array();
        // split by range seperator
		 vRangeVal = vLine.split("~");
		// check if selected range matches 
		if (vSelectedRange == vRangeVal[0])
		{
			var vSelected = vRangeVal[1]; 
			// add selected values
			var vArrSizes = vSelected.split("|");
			var arrayLength1 = vArrSizes.length;
			for (var y = 0; y < arrayLength1; y++) {
				var vSize = vArrSizes[y];
				 jQuery('#gapFrameModelSize').append( '<option value="'+vSize+'">'+vSize+'</option>' );
				 // also update Type in description
				 updateMachineType();
			}
		}
    }
}
// ENGMASA : Added for gapFrameModel ranges : END

// ENGMASA : Added below code to display Workspace description fields : START
function updateMachineType()
{
	// update machineframe type sseperatly
	var vDescriptionVal = jQuery("#DescriptionId").val();
	var descrName = "DescriptionId";
	// handle for edit form
	if(vDescriptionVal)
	 {
		 //
	 }
	 ...
	 {
		 vDescriptionVal = jQuery("#Description").val();
		 descrName = "Description";
     }

	 if(vDescriptionVal != "undefined" && vDescriptionVal != "null" && vDescriptionVal != "" && vDescriptionVal.length > 0)
	 {
		var vTextArea = vDescriptionVal.split("\n");
		var vFrameMachineType =  jQuery("#gapFrameMachineType").val();
		var vFrameModel = jQuery("#gapFrameModel").val();
		var vFrameSize = jQuery("#gapFrameModelSize").val();
		if (!vFrameModel || vFrameModel=="-")
		{	
			vFrameModel = "";
		}
		if (!vFrameSize || vFrameSize=="-")
		{	
			vFrameSize = "";
		}
		if (!vFrameMachineType || vFrameMachineType=="-")
		{	
			vFrameMachineType = "";
		}
		if(vFrameMachineType != "undefined")
		{
			 for (var i = 0; i < vTextArea.length; i++) {
				 var vVal = vTextArea[i];
				 var vTemp    = vVal.split("-");
				 var vFirstHalf = trimString(vTemp[0]);
				 if (vFirstHalf=="Type")
				 {
					 var vNewVal = vFirstHalf+ " - "+ vFrameModel+vFrameSize+vFrameMachineType;
					 vTextArea[i] = vNewVal;
					 var vNewDesc = vTextArea.join("\n");
			
					 jQuery("#"+descrName).val(vNewDesc);
					 jQuery("#"+descrName).attr("readonly", "readonly");
					 break;
				 }
			 }
		}	
...
function updateWorkspaceDesc(vFieldVal)
{
	var descrName = "DescriptionId";
	//alert("vFieldVal : "+vFieldVal);
	 var vDescriptionVal = jQuery("#DescriptionId").val();
	// alert(vDescriptionVal);
	// handl edit form
	if(vDescriptionVal)
	 {
		 //
	...
	// alert(vDescriptionVal);
	 ...
	 {
		var vTextArea = vDescriptionVal.split("\n");
		var vField    = vFieldVal.split("~");
		var vFieldSeq = vField[0];
		var vFieldLabel = vField[1];
		var vFieldName = vField[2];				
		
		var vValueToBeUpdate = jQuery("#"+vFieldName).val();
		
		for (var i = 0; i < vTextArea.length; i++) {
				 var vVal = vTextArea[i];
				 var vTemp    = vVal.split("-");
				 var vFirstHalf = trimString(vTemp[0]);
				 if (vFirstHalf==vFieldLabel)
				 {
					 var vNewVal = vFirstHalf+ " - "+ vValueToBeUpdate;
					 vTextArea[i] = vNewVal;
					 var vNewDesc = vTextArea.join("\n");
		...
		/* var vFieldToBeModified = vTextArea[vFieldSeq];
		// split by hypen
		var vTemp    = vFieldToBeModified.split("-");
		var vFirstHalf = trimString(vTemp[0]);
		
		// check if firts half matches label then only update
		if (vFirstHalf==vFieldLabel)
		{
			var vNewVal = vFirstHalf+ " - "+ vValueToBeUpdate;
			
			 vTextArea[vFieldSeq] = vNewVal;
			 var vNewDesc = vTextArea.join("\n");
			
			 jQuery("#DescriptionId").val(vNewDesc);
		} */
		
	 }
}
// ENGMASA : Added below code to display Workspace description fields : END

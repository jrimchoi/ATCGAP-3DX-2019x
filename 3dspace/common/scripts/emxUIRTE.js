...
emxUIRTE.RTE_SUP_TAGS = ["br","strong","em","b","i","u", "p","sup","sub","strike","s"];
...
//spellchecker attribute sc_dojoType
	...
	jQuery(self.iframe_doc).bind('paste', function(e){
	    ...
	    window.setTimeout(function(){ jQuery(e.target).trigger(e.type);},0);
	  });

	jQuery(self.iframe_doc).bind('afterpaste', function() {
		jQuery('body', self.iframe_doc).html(cleanup_word(jQuery('body', self.iframe_doc).html(), true, true, true));
	});
...
lwRTE.prototype.update_hidden_field = function() {
	...
	if(jQuery('body', self.iframe_doc).text().trim() != ""){
		...
		/* #RTL start ris */	
		var content = ifhtml.replace(/<[\/]{0,1}(span|A)[^><]*>/gi,"").replace(/&nbsp;/gi, " ").replace(/&lt;/gi, "<").replace(/&gt;/gi, ">").replace(/&amp;/gi, "&").replace(/<br>/gi, "<br/>");

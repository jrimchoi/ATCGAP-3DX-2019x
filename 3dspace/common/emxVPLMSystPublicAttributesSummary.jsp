<%--  emxVPLMSystPublicAttributesSummary.jsp   -   page for Public attributes of VPLMEntities
   ...
   static const char RCSID[] = $Id: emxVPLMSystPublicAttributesSummary.jsp.rca 1.12.1.1.1.2 Sun Oct 14 00:27:58 2007 przemek Experimental cmilliken przemek $
--%>
...
<%@ page import = "com.dassault_systemes.vplm.ProductionSystemNav.interfaces.IVPLMProductionSystemNav" %>
...
<%
	...
	IPLMxCoreAccess _coreAccess=_session.getVPLMAccess(); 
   	...
   	// modeler System
     IVPLMProductionSystemNav modeler = (IVPLMProductionSystemNav)_session.getModeler("com.dassault_systemes.vplm.ProductionSystemNav.implementation.VPLMProductionSystemNav");
	...
	String[] plmidArray = null;
   	...
   	try
	{
		plmidArray = ((IVPLMProductionSystemNav) modeler).getPLMObjectIdentifiers(m1idList);
	}
	catch (Exception e){
		e.printStackTrace();
	}
    ...
    if (plmxresult.next())
    {
		...
		for (String shortName : dicoAttributes.keySet())
		{
			try
			{
				...
				formattedAttributeValue = VPLMJCommonUIDicoServices.formatValue(context, "VPLMSystemEditor", plmType, shortName, value, lang);
				out.println("<tr><td class=\"label\">" + translatedAttributeName + "</td>");
				out.println("<td class=\"field\">" + formattedAttributeValue + "</td>");
			}
			catch (Exception e)
	...
	out.println("</body>");
		...
		if (( e.toString() != null) && (e.toString().trim().length()>0))
    		{
			emxNavErrorObject.addMessage ("emxVPLMSystPublicAttributesSummary: The supplied object is invalid");
		}
	      e.printStackTrace();
   } finally {
		if (_session != null) {

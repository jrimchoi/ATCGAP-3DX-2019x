...
<html>
    ...
    String curType 	= UINavigatorUtil.getI18nString("emxVPLMSynchroCfg.Info.ProductLine", "emxVPLMSynchroCfgStringResource", request.getHeader("Accept-Language"));
  	String title  	= UINavigatorUtil.getI18nString("emxVPLMSynchroCfg.Info.UnpublishInputTitle", "emxVPLMSynchroCfgStringResource", request.getHeader("Accept-Language"));
    ...
    <body class="slide-in-panel" onload="turnOffProgress();">
		...
		<div id="divPageBody" <%if(isIE){%> style="top:85px;" <%}%>>
			<iframe name="syncDisplayFrame" src="../common/emxSynchronizeWithVPLM_Evolution_UIUnpublishForm.jsp?id=<%=XSSUtil.encodeForURL(context,sObjID)%>&type_var=<%=curType%>&targetLocation=<%=XSSUtil.encodeForURL(context,targetLocation)%>"></iframe>
		</div>

...
<%
	...
	String i18_title = getSafeNLS("Access.Policy.Result");
	//get entered parameters
	String policyPattern = request.getParameter("policyPattern");
	
	//pack result as argument to a JPO method
	HashMap hMap = new HashMap();
	hMap.put("policyPattern", policyPattern);
	String [] progArgs;

	try{
		progArgs = JPO.packArgs(hMap);
		//set list of resulted policies through a JPO
		JPO.invoke(context,"emxPLMOnlineAdminPersonViewProgram", null,"setPolicyList", progArgs);

        // forwarding to emxTable JSP
%>
        <jsp:forward page="../common/emxTable.jsp">
            <jsp:param name="table"       value="emxPLMOnlineAdminPolicyResultTable"/>
            <jsp:param name="objectBased" value="false"/>
            <jsp:param name="header"      value="<%=i18_title%>"/>
            <jsp:param name="program"     value="emxPLMOnlineAdminPersonViewProgram:getPolicyNamesForTable"/>
        </jsp:forward>
<%
	}
	catch(Exception e)

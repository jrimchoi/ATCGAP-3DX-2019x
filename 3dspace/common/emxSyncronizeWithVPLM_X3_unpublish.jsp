...
<html>
	...
	int    VPMsize 				= 0;
		...
		String productStatus  = request.getParameter("chkProduct");
			if(productStatus != null){syncVersion += "product";}
		String diversityStatus  = request.getParameter("chkDiv");
			if(diversityStatus != null){syncVersion += "diversity";}
		String LINStatus  		= request.getParameter("chkBOM");
			if(LINStatus != null){syncVersion += "LIN";}
		String prodConfStatus  	= request.getParameter("chkPC");
			if(prodConfStatus != null){syncVersion += "ProdConf";}
		
        // Invoke the JPO
			if(!syncVersion.equals("")){
	        ...
	        Map matrixObjIDvplmObjIDMap = (Map)JPO.invoke(context, "VPLMIntegSyncWithVPLM", null, "execute", args, Map.class);
	            ...
	            String operationStatus = (String)matrixObjIDvplmObjIDMap.get(VPLMIntegrationConstants.MAPKEY_OPERATION_STATUS);
                	...
                	//errConcat +=  errPLMID;
                	//Exception e = new Exception(errMessage+" on PLMID : "+errPLMID);

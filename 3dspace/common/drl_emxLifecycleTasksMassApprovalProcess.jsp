...
<% 
    ...
    try {
		...
		if("true".equals(processRouteAssignee)){
			...
			HashMap requestParams 	= UINavigatorUtil.getRequestParameterMap(pageContext);
			drRouteUtil routeUtil=new drRouteUtil(context);
			final Logger log = drLogger.getLogger(this.getClass());
			try{
				boolean completed = routeUtil.updateAssigneesOnRouteTasksInTasksView(JPO.packArgs(requestParams));				
			}catch(Exception ex){
			...
			//cleanup sessionn level tablebean
        ...
        for (Iterator itrObjectList = objectList.iterator(); itrObjectList.hasNext();) {
			...
			if(strApprovalStatusAction == null){
				strApprovalStatusAction = (String)mapCurrObject.get("Approval Status" + strSerialNumber);
			}
			
            strComments             = emxGetParameter(request, "Comments" + strSerialNumber);
			if(strComments == null){
				strComments = (String)mapCurrObject.get("Comments" + strSerialNumber);
            }
			//END : 3dxtools routes enhancement

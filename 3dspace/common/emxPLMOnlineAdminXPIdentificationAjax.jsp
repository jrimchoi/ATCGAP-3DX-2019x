...
%>
<%--
//@fullReview  ZUR - 14/08/04 - IR-308845-3DEXPERIENCER2015x - Split from emxPLMOnlineAdminXPApplicationsAjax.jsp
--%>
...
for (int i=0 ; i<nbParameters ; i++)
{
	...
	for (int j=0 ; j<nbOfArguments ; j++)
	{
		...
		//System.out.println("STARTXXXXX::"+argumentID+"YYYYY::"+argumentValue+"::END");
		
		if ("IdentifierAffix".equalsIgnoreCase(parameterIDs[i]))
		{
			AppIntUIConnector.IExchangeDomain identificationDomain = appIntUIConnector.getDomain("ObjectIdentification");           	
	        AppIntUIConnector.IExchangeFamily[] identificationFamilies = identificationDomain.getFamilies();       
	    
	        for (int k=0; k<identificationFamilies.length; k++)
			{			
				if ("ObjectIdentifier".equals(identificationFamilies[k].getID()))
				{
					AppIntUIConnector.IExchangeParameter[] vCustoNamingParamsList = identificationFamilies[k].getParameters();	    	   
					
					for (int kl=0; kl<vCustoNamingParamsList.length; kl++)
					{
						String rParamID = vCustoNamingParamsList[kl].getID();		
						
						if ("IdentifierAffix".equalsIgnoreCase(rParamID))
						{								   
					    	AppIntUIConnector.IExchangeArgument[] namingAffixArgs = vCustoNamingParamsList[kl].getArguments();
					    	
					    	if ("SeparatorArg".equalsIgnoreCase(argumentID))
					    	{					    	
					    		for (int lm=0; lm<namingAffixArgs.length; lm++)
					    		{
					    			if ("SeparatorArg".equals(namingAffixArgs[lm].getID()))
					    			{
					                	//String[] argValList = namingAffixArgs[lm].getValuesList();     
										String cteVal = "-";										
										/*
					            		for (int mn=0; mn<argValList.length; mn++)
					                	{ */  
										AppIntUIConnector.IExchangeConstant matchCte =  vCustoNamingParamsList[kl].getConstant(argumentValue);            		
					            		cteVal = matchCte.getValue();   
					            		//System.out.println("CONVERSE::"+cteVal+"<::>"+argumentValue+"<::>");//argValList[mn]
					            		argumentValue = cteVal;
					                	//cte(:)	ValargValList[j] (Colon)   
					                	/*break;
					                	}*/
						            	break;		
					    			}

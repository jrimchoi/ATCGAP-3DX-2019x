...
<%	
	final String action = emxGetParameter(request, DCLConstants.DCL_ACTION);
	...
	try 
	{
		...
		{
           throw new Exception("Error: Mandatory parameter 'dclAction' missing.");
        }
		...
		strJavaScript = (String)mResponse.get(DCLConstants.ACTION_JAVASCRIPT);
		strInclude = (String)mResponse.get(DCLConstants.ACTION_INCLUDEJSP);
		saIncludeJS = (String[])mResponse.get(DCLConstants.ACTION_INCLUDEJAVASCRIPT);
		strAjax = (String)mResponse.get(DCLConstants.ACTION_AJAX);
	}
	catch (Exception exp) {
		exp.printStackTrace();

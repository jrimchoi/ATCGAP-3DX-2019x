<%--  emxParameterCreateProcess.jsp
   ...
   static const char RCSID[] = $Id: emxParameterCreateProcess.jsp.rca 1.6.2.2.1.4 Wed Oct 22 15:48:29 2008 przemek Experimental przemek $
--%>
...
<jsp:useBean id="createBean" class="com.dassault_systemes.parameter_ui.UIParameterForm" scope="session" />
...
<script language="javascript" src="emxParameterUIFormUtil.js"></script>
...
<%
        ...
        // is the relationship compositional?
        boolean relCompositional = false;
        String direction = emxGetParameter(request, "direction");
        String relationship = emxGetParameter(request, "relationship"), relSymb = "";
        if (relationship != null && !relationship.equals("") && !relationship.equals("*")) {
            relSymb = PropertyUtil.getSchemaProperty(context, relationship);
            if("".equals(relSymb))
        ...
        if ("from".equalsIgnoreCase(direction))
        {
        	...
        	mql.executeCommand(context, "print relation $1 select $2 dump", relSymb, "compositional");
            String compStr = mql.getResult();
            if (compStr.trim().equalsIgnoreCase("true"))
            	relCompositional = true;
        }
		
        ContextUtil.startTransaction(context, true);
		newObjectId = createBean.createObject(context, requestMap, fields, timeZone);
		// connect if objectId is passed.
		if (!relCompositional && objectId != null && !"".equals(objectId)) {
		...
		else if (relCompositional && !newObjectId.isEmpty() && !"".equals(newObjectId))
	...
	if(topWindow.getTopWindow){
	 contentFrame = findFrame(topWindow.getTopWindow(), "content"); 
		...
		contentFrame = findFrame(getTopWindow(), "content"); 
	            ...
	            if(contentFrame)
                {
                    contentFrame.document.location.href = "../common/emxTree.jsp?objectId=<%=XSSUtil.encodeForURL(context, newObjectId)%>&emxSuiteDirectory=<%=XSSUtil.encodeForURL(context, emxSuiteDirectory)%>";
                }
...
<%
            ...
            {
%>
      		modalWin  = showModalDialog("../common/emxTree.jsp?objectId=<%=XSSUtil.encodeForURL(context, newObjectId)%>", 700, 600, true, true);      		

<%--  ` - Results page for Object Count Report
   Copyright (c) 2005-2018 Dassault Systemes.
   All Rights Reserved.
   This program contains proprietary and trade secret information of MatrixOne,Inc.
   Copyright notice is precautionary only
   and does not evidence any actual or intended publication of such program
   static const char RCSID[] = $Id: emxMetricsObjectCountReportResults.jsp.rca 1.41 Wed Oct 22 16:11:55 2008 przemek Experimental $
--%>
...
<%
        ...
        else if(!"true".equals(strFromDisplayFormat)) // if reportView mode, then read the results from webreport and display the results
        {
...
//        if(strContainedIn == null || "null".equalsIgnoreCase(strContainedIn) || "".equalsIgnoreCase(strContainedIn)){
//            strContainedIn = "";
//        }
        ...
        sbSearchCriteria=metricsReportBeanUI.formSearchCriteria(context, strActualContainedIn, strType, strName,strLatestRev, whereClause, strVault, strVaultSelction, strRevision );

        if(strGroupBy.equalsIgnoreCase("type")) {
            sbFirstGroupBy.append(strGroupBy);
        }
        else if(strGroupBy.equalsIgnoreCase("owner")) {
            sbFirstGroupBy.append(strGroupBy);
        }
        else if(strGroupBy.equalsIgnoreCase("current")) {
            sbFirstGroupBy.append(strGroupBy);
        }
        else if(strGroupBy.equalsIgnoreCase("vault")) {
            sbFirstGroupBy.append(strGroupBy);
        }
        else{
            boolValue = metricsReportBean.getAttributeType(context,strGroupBy);
            if(boolValue.trim().equalsIgnoreCase("timestamp")){
        ...
        if(boolValue.trim().equalsIgnoreCase("timestamp")){
            Expression groupByExpr3 = new Expression();
            groupByExpr3.setValue(sbFirstGroupBy.toString());
            groupByExprList.addElement(groupByExpr3);
        }else{
...
<%
            if(!"Tabular".equalsIgnoreCase(strChartType))
            {
                ...
                String groupByAttrType = "Attribute";
                String subGroupByAttrType = "Attribute";
                String strSubGroupDataType = "";
                StringList attrRanges = new StringList();
                if(strGroupBy!=null && (strGroupBy.equals("Owner") || strGroupBy.equalsIgnoreCase("Current") || strGroupBy.equalsIgnoreCase("Vault")))
                {
                    groupByAttrType = "basic"; 
                }
                if(strGroupBy!=null && (strGroupBy.equalsIgnoreCase("Type")))
                {
                    groupByAttrType = "type";
                }
                String modGroupByAttrType = groupByAttrType;
                if(strGroupBy!=null && strGroupBy.equalsIgnoreCase("Current"))
                {
                    modGroupByAttrType = "state";
                }
                else if(strGroupBy!=null && (strGroupBy.equalsIgnoreCase("Vault")))
                {
                    modGroupByAttrType = "vault";
                }
                else if(strGroupBy!=null && (strGroupBy.equalsIgnoreCase("Owner")))
                {
                    modGroupByAttrType = "person";
                }
                if(strGroupBy!=null && !"".equals(strGroupBy) && groupByAttrType.equals("Attribute"))
                {
                    try
                    {
                        AttributeType attrType = new AttributeType(strGroupBy);
                        attrType.open(context);                        
                        StringList strChoiceList = attrType.getChoices(context);
                        if(strChoiceList!=null && strChoiceList.size()>0)
                        {
                            modGroupByAttrType = "Range";
                        }                        
                    }
                    catch(Exception e)
                    {
                    ...
                    while(keyItr.hasNext())
                    {
                        ...
                        if(!key.equalsIgnoreCase("CellNumber") && !key.equalsIgnoreCase(metricsReportBean.SUMMARY_RESULT_LIST))
                        {
                            ...
                            if(strGroupBy.equalsIgnoreCase(key)){
                                ...
                                StringList strList = FrameworkUtil.split(value,",");
                                Iterator strListIter = strList.iterator();
                                int count = 0;
                                while(strListIter.hasNext())
                                {
                                    String actualValue = (String)strListIter.next();
                                    String modifiedValue = metricsReportBeanUI.internationalizedValue(context,strType,"",strGroupBy,actualValue,modGroupByAttrType);
                                    if(count==0)
                                    {
                                    ...
                                    {                                       
                                       modifiedValues = modifiedValues + "," + modifiedValue;
                                    }
                                    count++;
                                }
                                if(key.equalsIgnoreCase("Type")) 
                                {
                                    key = metricsReportBeanUI.internationalizedValue(context,strType,"",strGroupBy,key,"basic");
                                }
                                else
                                {
                                    key = metricsReportBeanUI.internationalizedValue(context,strType,"",strGroupBy,key,groupByAttrType);
                                }
                                xValList.add(modifiedValues);
                                xTitleList.add(key);
                            }else{
                ...
                if("integer".equalsIgnoreCase(strSubGroupDataType))
                {
                    ...
                    while(yTitleIter.hasNext())
                    {
                        ...
                        String strKey = (String)yTitle.toString();
...
<%
            ...
            else
            {
                ...
                if(strSubGroup !=null && !"".equals(strSubGroup)){
                    strMessageText = metricsReportBean.substituteMacroValue(context,"emxMetrics.label.ObjectCountTabularFormatMessageWithSubGroup",reportDetails);
                }
                ...
                StringList rowHeaders  = metricsReportBeanUI.prepareObjectCountRowHeaders(context,chartDetailsMap,strType,strGroupBy);

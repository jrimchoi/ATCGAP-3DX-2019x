...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    ...
    <xsl:template name="booleanField">pageControl.arrFormVals[<xsl:value-of select="position()-1"/>] = new Array("<xsl:value-of select="@name"/>","<xsl:value-of select="."/>",<xsl:value-of select="@checked"/>,<xsl:value-of select="@disabled = 'true'"/>);</xsl:template>

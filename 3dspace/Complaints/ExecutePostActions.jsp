...
<jsp:include page="../TicketManagement/ExecutePostActions.jsp" />
...
function actionRefreshComplaint() {
	var contentFrame = findFrame(getTopWindow(), "content");
	contentFrame.location.href = contentFrame.location.href;
}
 ...
 */
function submitComplaintProperties(complaintId,flag1,flag2,flag3,flag4){
	...
	var contentFrame = findFrame(getTopWindow(), "_self");
	contentFrame.location.href = url;	
}
function openAddDerivedEventForm(objectId) {
	var submitURL = "../common/emxCreate.jsp?form=type_DerivedEvent&nameField=autoName&type=type_DerivedEvent&policy=policy_ComplaintEvent&mode=create&createJPO=com.dassault_systemes.enovia.lsa.complaint.services.ui.Event:createDerivedEvent&header=LPQ.Event.Command.Label.AddDerivedEvent&suiteKey=Complaints&showApply=true&submitAction=refreshCaller&HelpMarker=emxhelpeventaddchild&objectId="+objectId;
...
function addExistingProductModelToEvent(strSelectedId)
{
	 var submitURL = "../common/emxForm.jsp?form=AddExistingProductModel&mode=edit&formHeader=FTComplaints.AddProduct.Header&submitAction=refreshCaller&postProcessJPO=com.dassault_systemes.enovia.lsa.complaint.services.ui.Event:AddProductModel&suiteKey=Complaints&HelpMarker=emxhelpeventaddmodel&objectId="+strSelectedId;
	getTopWindow().showSlideInDialog(submitURL, "true"); 
	
}
function addExistingProductModelToEventError()
{
	var message ="Product is already attached to Event.";
    alert(message);
    getTopWindow().window.closeSlideInDialog();
}

function addPatientToEvents(objectIds) {
	var submitURL = "../common/emxCreate.jsp?form=CreatePatient&type=type_Patient&nameField=autoName&targetLocation=slidein&postProcessJPO=com.dassault_systemes.enovia.lsa.complaint.services.ui.Patient:createPatientPostProcess&submitAction=refreshCaller&suiteKey=Complaints&HelpMarker=emxhelpeventaddpatient&ObjectIds="+objectIds;
	getTopWindow().showSlideInDialog(submitURL, "true");
}

function actionShowAlertMessageAndCloseDialogue(errorMessage)
{
	alert(errorMessage);
	getTopWindow().closeShortcutDialog();
}

function alertForUnAssignedView(){
	alert("<%=getI18NString(context,"LPQ.Complaint.Message.Error.UnAssignedView")%>"); //XSSOK
}
	</script>

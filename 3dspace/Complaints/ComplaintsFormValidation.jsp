...
function validateRiskAnalysisDetailsJSHint()
{
...
function enableFormFields(){
	...
	function preprocessEnableOrDisableEdit()
	{
		var url = "../common/iwCSAjaxController.jsp" + "?" + "listProgramName=" + "com.dassault_systemes.enovia.lsa.ticket.services.ui.ComplaintProductEvaluation:checkAccessOnField";
		if (window.XMLHttpRequest) {
	        request = new XMLHttpRequest();
	    }
	    else if (window.ActiveXObject) {
		...
		request.open("GET", url, true);
		request.onreadystatechange = function () { enableOrDisableField(request); }
		request.send(null);
	}
	function enableOrDisableField(request)
	{
		if (request.readyState == 4) {
	        if (request.status == 200) {
	            // handle the response js object
	            ...
	            var result = jsObject.result;
	            var vHasCIRole = result[0].value;
	            var vHasQMAndRURole = result[1].value;
	            var vHasQMRole = result[2].value;
	            var vHasRURole = result[3].value;
	            var fieldArray = ["ComplaintInvestigationRequired","ComplaintInvestigationClosedDate","ComplaintInvestigationLocation",
	                                "DeviceDeficiencyConfirmed","ComplaintInvestigationMethods","ResultsCode","ConclusionCode",
	                                "ComplaintInvestigationRecommendedActions","ComplaintNoInvestigationRationale","ExternalEvaluator",
	                                "ManufacturerPreliminaryAnalysis","ManufacturerAnalysis","Comments","ComplaintInvestigationReviewer",
	                                "ComplaintInvestigationReviewDate","ComplaintRiskAnalysisRequired","ComplaintRiskAnalysisNumber",
	                                "ComplaintNoRiskAnalysisRationale","ComplaintInvestigationConclusionSummaryMalfunction",
	                                "ComplaintInvestigationConclusionSummaryLabeling","ComplaintInvestigationConclusionSummaryUseError",
	                                "ComplaintInvestigationConclusionSummary","Severity","FurtherInvestigationRequired","RemedialActionTaken",
	                                "ComplaintInvestigationCAPANumber","Notes"];
	            if("TRUE"==vHasCIRole)
	            	{
	            		return true;
	            	}
	            else if("TRUE"==vHasQMAndRURole)
	            	{
	        	  		var fieldArrayQMAndRU = [fieldArray[0],fieldArray[1],fieldArray[2],fieldArray[3],fieldArray[4],fieldArray[5],fieldArray[6],
	        	  		                 fieldArray[8],fieldArray[9],fieldArray[10],fieldArray[11],fieldArray[13],fieldArray[14],
	        	  		                fieldArray[15],fieldArray[16],fieldArray[17],fieldArray[18],fieldArray[19],fieldArray[20],fieldArray[21]];
	            		setFormFieldEditAccess(fieldArrayQMAndRU, false);
	            	}
	            else if("TRUE"==vHasQMRole)
	            	{
	        	  		var fieldArrayQM = [fieldArray[0],fieldArray[1],fieldArray[2],fieldArray[3],fieldArray[4],fieldArray[5],fieldArray[6],
	        	  		                  fieldArray[7],fieldArray[8],fieldArray[9],fieldArray[10],fieldArray[11],fieldArray[12],fieldArray[13],fieldArray[14],
	        	  		                fieldArray[15],fieldArray[16],fieldArray[17],fieldArray[18],fieldArray[19],fieldArray[20],fieldArray[21]];
	            		setFormFieldEditAccess(fieldArrayQM, false);
	            	}
	         	else if("TRUE"==vHasRURole)
	            	{
	        	   	 	var fieldArrayRU = [fieldArray[0],fieldArray[1],fieldArray[2],fieldArray[3],fieldArray[4],fieldArray[5],fieldArray[6],
	          	  		                  fieldArray[8],fieldArray[9],fieldArray[10],fieldArray[11],fieldArray[13],fieldArray[14],fieldArray[15],
	          	  		                fieldArray[16],fieldArray[17],fieldArray[18],fieldArray[19],fieldArray[20],fieldArray[21],fieldArray[22],
	        	   	 					fieldArray[23],fieldArray[24],fieldArray[25],fieldArray[26]];
	        	  		 setFormFieldEditAccess(fieldArrayRU, false);
	            	}
			}
	        request = null;
	    }
	...
	function enableAllFields()
	{
		var fieldArray = ["ComplaintInvestigationRequired","ComplaintInvestigationClosedDate","ComplaintInvestigationLocation",
	                      ...
	                      "ComplaintNoRiskAnalysisRationale","ComplaintInvestigationConclusionSummaryMalfunction",
	                      "ComplaintInvestigationConclusionSummaryLabeling","ComplaintInvestigationConclusionSummaryUseError",
	                      "ComplaintInvestigationConclusionSummary","Severity","FurtherInvestigationRequired","RemedialActionTaken","ComplaintInvestigationCAPANumber","Notes"];	
	...
	function setFormFieldEditAccess(fieldArray, disable){
		...
		function setRevisionForType()
		{
			var type = document.getElementById("TypeId");
			var selectedIndex = type.options.selectedIndex;
			var selectedText = type.options[selectedIndex].value;
		...
		function outcomeTextField()
		{
			var vOutcome = document.getElementById("OutcomeId");
			var vOutcomeTA = document.getElementById("OutcomeTextArea");
			var vOutcome_calc = document.getElementById("calc_OutcomeTextArea");
			
			var outcome = vOutcome.value;
			var index = outcome.indexOf("Other");
			var outcomes = outcome.split(":",2);
			
			if(index==1 || index==0 ) {
				vOutcomeTA.disabled = false;
				 if (!(vOutcomeTA.value == null || (typeof vOutcomeTA ==="undefined") || typeof outcomes[1] ==="undefined")) {
					vOutcomeTA.value = outcomes[1];
				 }
				vOutcome_calc.children[0].className = 'labelRequired';
			} else {
				vOutcomeTA.disabled = true;
				vOutcome_calc.children[0].className = 'createLabel';
			}
		}
		...
		function outcomeTextArea() {
			...
			if(index==1 || index==0 ) {
				...
				alert("<%=getI18NString(context,  "LPOComplaint.Message.Fieldsinreditalicsarerequired")%>"); //XSSOK
					return false;
				}
				else {
					return true;
		...
		function validateFormAdverseEventReportAdditionalFieldContactPhone()
		{
			var vHome = document.getElementById("ContactPhone").value;
				if(vHome != null && vHome != "" )
				{
					if(!checkForNoLetters(vHome))
					{
						alert("<%=getI18NString(context,"LPQ.AER.ContactPhone.AlphabetsPresent.ErrorMessage")%>"); //XSSOK
						return false;
					}
		...
		//added by j48 for patient death field validation
		
		function validatePatientDeathfield()
		{
	    if (this.value != null && this.value != "") {
	        if(typeof document.getElementsByName(this.name + "_msvalue")[0] ==="undefined"){
	       	 return true;
	        }
	    	var formFieldDateMS = document.getElementsByName(this.name + "_msvalue")[0].value;
	           ...
	           if(msForm > msNow)
	           {
	            	 alert("<%=getI18NString(context, "LPOComplaint.Common.DateCannotBeGreaterThanToday")%>"); //XSSOK
	 			   return false;
	           }
	           
	           var dateOfBirth_num = document.getElementsByName("DateOfBirth" + "_msvalue")[0].value;
	           var formFieldDateOfBirth_num = dateOfBirth_num - 43200000;
	           var birthDate = new Date(formFieldDateOfBirth_num);
	           birthDate.setHours(0);
	           birthDate.setMinutes(0);
	           birthDate.setSeconds(0);
	           birthDate.setMilliseconds(0);
	           
	           var msBirth = birthDate.valueOf();
	           
	           if(msBirth > msForm)
	           {
	            	 alert("<%=getI18NString(context, "LPOComplaint.Common.DeathDateCannotBeGreaterThanDOB")%>"); //XSSOK
	 			   return false;
	           }
	       }    
	     else{
	    	 var DeathId = document.getElementById("DeathId");
	         var DeathStatus =DeathId.value;
	         if(DeathStatus=="TRUE")
	         {
	        	 alert("<%=getI18NString(context, "LPOComplaint.Common.DeathDateCannotBeEmpty")%>"); //XSSOK
	        	 return false;
	         }
	     }
		...
		function enableRequiredFeilds() {
		    isDeathDateRequired();
		    isRemedicalActionTakenDiagnosisResultsAndNoDiagnosisRationaleRequired();
		    isDeathDueToAutopsyAndResultsRequired();
		    outcomeTextField();
		}

		function isDeathDateRequired(){
				var vDeath = document.getElementById("DeathId");
				
				if(vDeath.value == null ) {
					emxFormDisableField("DeathDate", false);
				}
				else if(vDeath.value == "FALSE" || vDeath.value == "No") {
					emxFormDisableField("DeathDate", false);
				}
				else{
					emxFormDisableField("DeathDate", true);
				}
			}
		function isRemedicalActionTakenDiagnosisResultsAndNoDiagnosisRationaleRequired() {
				var vDiagnosis = document.getElementById("DiagnosisId");
				var vRemedicalActionTaken = document.getElementsByName("RemedialActionTakenDisplay")[0];
				var remediationChooser = document.getElementsByName("btnRemedialActionTaken")[0];
				var vDiagnosisResults = document.getElementById("DiagnosisResults");
				var vNoDiagnosisRationaleRequired = document.getElementById("NoDiagnosisRationale");
				if(vDiagnosis.value == null) {
					vRemedicalActionTaken.disabled = true;
					vDiagnosisResults.disabled = true;
					vNoDiagnosisRationaleRequired.disabled = true;
					remediationChooser.disabled = true;
				}
				else if(vDiagnosis.value=="TRUE" || vDiagnosis.value=="Yes") {
					vRemedicalActionTaken.disabled = false;          
					vDiagnosisResults.disabled = false;                
					vNoDiagnosisRationaleRequired.disabled = true;
					remediationChooser.disabled = false;
				}
				else {
					vRemedicalActionTaken.disabled = true;           
					vDiagnosisResults.disabled = true;                
					vNoDiagnosisRationaleRequired.disabled = false;
					remediationChooser.disabled = true;
				}
			}	
		function isDeathDueToAutopsyAndResultsRequired() {
				var vAutopsyPerformed = document.getElementById("AutopsyPerformedId");
				var vDeathDueToAutopsy = document.getElementById("DeathDueToAutopsyId");
				var vResultsRequired = document.getElementById("Results");
				if(vAutopsyPerformed.value == null) {
					vDeathDueToAutopsy.disabled = true;
					vResultsRequired.disabled = true;
				}
				if(vAutopsyPerformed.value=="TRUE" || vAutopsyPerformed.value=="Yes") {
					vDeathDueToAutopsy.disabled = false;
					vResultsRequired.disabled = false;	
				}
				else {
					vDeathDueToAutopsy.disabled = true;
					vResultsRequired.disabled = true;  
		...
		function validateMedical() 
		{
			...
			var ReportableStatus = document.getElementsByName("ReportableStatus")[0].value;
			 if (ReportableStatus.toLowerCase() == "reportable")
					document.getElementsByName("Medical")[0].disabled = true;
		...
		function validateReportableRationale() 
		{

				var ReportableStatus =  document.getElementsByName("ReportableStatus")[0].value;
				var ReportableRationale = document.getElementsByName("ReportableRationale")[0].value;
			  if ((ReportableStatus.toLowerCase() == "reportable" || ReportableStatus.toLowerCase() == "not reportable" ) && ReportableRationale.toLowerCase() == "")
					{
						 alert("<%=getI18NString(context,  "LPQ.Complaint.Message.Error.MustEnterReportableRationale")%>"); //XSSOK
					return false;
					}
			  return true;
		}

...
<%@page import="com.dassault_systemes.enovia.lsa.LSACommonExecuteCallable"%>
...
<%
	...
	String lsaWebAppPath = getServletConfig().getServletContext().getRealPath("/LSA");
	parameterMap.put("lsaWebAppPath", new String[] {lsaWebAppPath});
	if(lsaWebAppPath.contains("\\")) {
		lsaWebAppPath = lsaWebAppPath.replace("\\","\\\\");
	}
	...
	context.setCustomData("lsaWebAppPath", lsaWebAppPath);
%> 	<input type="hidden" name="lsaWebAppPath"
			value="<xss:encodeForHTMLAttribute><%=lsaWebAppPath%></xss:encodeForHTMLAttribute>" />
...
// Added for Widget Platform Adoption
function showSearchDialog(newParameters) {
	var strFullSearchURL = "../common/emxFullSearch.jsp?";
	for(var parameterName in newParameters) {
		strFullSearchURL += "&"+parameterName+"="+newParameters[parameterName];
	}
	showModalDialog(strFullSearchURL);
}
</script>
<%
	...
	String strURL = null;
	String[] parameterMapSuiteKey = (String[])parameterMap.get("executeSuiteKey");
	...
	try
	{
		...
		if(parameterMapSuiteKey != null){
				suiteKey=parameterMapSuiteKey[0];
		}
		...
		FrameworkUtil.validateMethodBeforeInvoke(context, programName, methodName, com.dassault_systemes.enovia.lsa.LSACommonExecuteCallable.class);
		//END

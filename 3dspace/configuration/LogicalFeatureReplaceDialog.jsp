<%--
  LogicalFeatureReplaceDialog.jsp
...
String type = EnoviaResourceBundle.getProperty(context,"Configuration.LogicalFeatureReplace.type_LogicalFeature");
...
<form name="ReplaceLogicalFeature" method="post" action=javascript:moveNext()><input type="hidden" name="sourceObjectId" value="<xss:encodeForHTML><%=strObjectId %></xss:encodeForHTML>" />
...
<script language="javascript" type="text/javaScript">
  //<![CDATA[
    var  formName = document.ReplaceLogicalFeature;

 function showProductChooser()
 {
     showModalDialog('../common/emxFullSearch.jsp?objectId=<%=XSSUtil.encodeForURL(context,strFeatureId)%>&txtType=<%=XSSUtil.encodeForURL(context,txtType)%>&field=TYPES=<%=XSSUtil.encodeForURL(context,strInclusionFeatureTypes)%>,<%=XSSUtil.encodeForURL(context,strInclusionProductTypes)%>:CURRENT!=policy_LogicalFeature.state_Obsolete,policy_Product.state_Obsolete&excludeOIDprogram=LogicalFeature:excludeAvailableLogicalFeatureForRepalce&excludeOID=<%=XSSUtil.encodeForURL(context,strProductId)%>,<%=XSSUtil.encodeForURL(context,strFeatureId)%>&table=FTRFeatureSearchResultsTable&Registered Suite=Configuration&showInitialResults=false&selection=single&showSavedQuery=true&hideHeader=true&HelpMarker=emxhelpfullsearch&submitURL=../configuration/SearchUtil.jsp?mode=Chooser&chooserType=CustomChooser&fieldNameActual=txtToFeatureId&fieldNameDisplay=txtSourceObject',850,630); 
 }
 ...
 function closeWindow()
 {
   ...
   function submitForm()
   {
    ...
    function submit()
    {
    	...
    	formName.action="../configuration/LogicalFeatureReplaceProcess.jsp?objectId="+strObjectID+"&objectIdTobeconnected="+strId+"&strProdId="+strProductID+"&RelId="+strRelID+"&strParentId="+strParentID;

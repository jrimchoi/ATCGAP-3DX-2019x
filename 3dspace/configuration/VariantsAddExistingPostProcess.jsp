...
<%
  ...
  {  
     ...
     String strContextNotSupportedForVariant = EnoviaResourceBundle.getProperty(context,"Configuration","emxConfiguration.Feature.Create.ContextNotSupportedForVariant",strLanguage);
    ...
    //If the selection are not made in Search results page  
     if(strArrSelectedTableRowId==null){
	     ...
	     <%
	         ...
	         StringList cfSubTypes = ProductLineUtil.getChildTypesIncludingSelf(context, ConfigurationConstants.TYPE_VARIANT);
	         ...
	         StringList coSubTypes = ProductLineUtil.getChildTypesIncludingSelf(context, ConfigurationConstants.TYPE_VARIANTVALUE);
             ...
             for(int x=0;x<strArrSelectedTableRowId.length;x++){
	             ...
	             if( !cfSubTypes.contains(strParentType) &&  !plSubTypes.contains(strParentType) && !prdSubTypes.contains(strParentType)&& !modSubTypes.contains(strParentType))
	             {
	            	 %>
	                 <script language="javascript" type="text/javaScript">
	                       alert("<%=XSSUtil.encodeForJavaScript(context,strContextNotSupportedForVariant)%>");
	                 </script>
		            <%

...
%>
<SCRIPT>
	showModalDialog("ProductConfiguratorFS.jsp?contextId=<%=XSSUtil.encodeForURL(context, contextId)%>&strProductContextId=<%=XSSUtil.encodeForURL(context,strProductContextId)%>&pcId=<%=XSSUtil.encodeForURL(context,pcId)%>&txtStartEffectivity=<%= XSSUtil.encodeForURL(context, strStartEffectivity) %>&strAction=view", 940, 770);
</SCRIPT>	
<% 
}
%>

<%--
  ConfigurationFeatureAddExistingPreProcess.jsp
...
<%
  ...
  {	  
      ...
      {
          if(mxType.isOfParentType(context,strParentType,ConfigurationConstants.TYPE_CONFIGURATION_OPTION))
          {
              %>
        	...
        	// List of Sub Types of Features that need to be searched for Add Existing.
        	  if(mxType.isOfParentType(context,strParentType,ConfigurationConstants.TYPE_CONFIGURATION_FEATURE))
              {
        		  //if(strParentKeyInType != null && !"".equals(strParentKeyInType) && !strParentKeyInType.equalsIgnoreCase("Blank")){
        			  strInclusionFeatureTypes = EnoviaResourceBundle.getProperty(context,"emxConfiguration.ConfigurationOption.AddExisting.Features.IncludeTypes");
        			  strformInclusionList = "PARENT_CONFIGURATION_FEATURE,DISPLAY_NAME";
        			  contextAccessFunction = "ConfigurationOption";
        		 // }
        		//  else{
        			//  strInclusionFeatureTypes = EnoviaResourceBundle.getProperty(context,"emxConfiguration.ConfigurationFeatureOption.AddExisting.Features.IncludeTypes");
        			  //strformInclusionList = "PARENT_CONFIGURATION_FEATURE,DISPLAY_NAME";
        		//  }
              }
        	  else{
        		  strInclusionFeatureTypes = EnoviaResourceBundle.getProperty(context,"emxConfiguration.ConfigurationFeature.AddExisting.Features.IncludeTypes");
              ...
              <script language="Javascript">
                  ...
                  var submitURL = "../common/emxFullSearch.jsp?objectId=<%=XSSUtil.encodeForURL(context,strObjectID)%>&txtType=<%=XSSUtil.encodeForURL(context,strParentType)%>&field=TYPES="+strFeatureTypes+":CURRENT!=policy_ConfigurationFeature.state_Obsolete&excludeOIDprogram=ConfigurationFeature:excludeAvailableConfigurationFeature&table=FTRConfigurationFeaturesSearchResultsTable&HelpMarker=emxhelpfullsearch&selection=multiple&showInitialResults=false&showSavedQuery=true&hideHeader=true&formInclusionList=<%=XSSUtil.encodeForURL(context,strformInclusionList)%>&context="+context+"&suiteKey=Configuration&submitURL=../configuration/ConfigurationFeatureAddExistingPostProcess.jsp?suiteKey=Configuration&StringResourceFileId=emxConfigurationStringResource&SuiteDirectory=configuration&objectId=<%=XSSUtil.encodeForURL(context,strObjectID)%>&parentOID=<%=XSSUtil.encodeForURL(context,parentId)%>&emxExpandFilter=1&isModel=<%=XSSUtil.encodeForURL(context,isModel)%>&parentForIRule=<%=XSSUtil.encodeForURL(context,parentForIRule)%>";

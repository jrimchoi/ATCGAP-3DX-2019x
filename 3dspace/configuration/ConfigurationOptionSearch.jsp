...
<%
     ...
     if(strMode==null){
         ...
         strURL.append("field=TYPES=type_ConfigurationFeature:CURRENT!=policy_ConfigurationFeature.state_Obsolete&table=FTRConfigurationFeaturesSearchResultsTable&HelpMarker=emxhelpfullsearch&showSavedQuery=true&hideHeader=true&formInclusionList=DISPLAY_NAME");        
         ...
         strURL.append("&submitURL=../configuration/ConfigurationOptionSearch.jsp?mode=submit");
         strURL.append("&fieldNameActual="+fieldAct);
         strURL.append("&fieldNameDisplay="+fieldDisp);
    %>
    ...
    getTopWindow().location.href = "<%=XSSUtil.encodeForJavaScript(context,strURL.toString())%>&delimiter="+dispCFFiled.getAttribute("delimiter");
    </SCRIPT>
    <%
    	...
    	var actCFFiled = getTopWindow().getWindowOpener().document.getElementById("<%=XSSUtil.encodeForJavaScript(context,fieldAct)%>");
    	var dispCFFiled = getTopWindow().getWindowOpener().document.getElementById("<%=XSSUtil.encodeForJavaScript(context,fieldDisp)%>");
    	dispCFFiled.value = "<%=XSSUtil.encodeForJavaScript(context,strVal.toString())%>";
    	actCFFiled.value = "<%=XSSUtil.encodeForJavaScript(context,strVal.toString())%>";

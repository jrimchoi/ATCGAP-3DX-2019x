<%--
  VariantsAddExistingPreProcess.jsp
...
<%
  ...
  {	  
      ...
      String strAddExistingOnVariantUnsupported = EnoviaResourceBundle.getProperty(context,"Configuration","emxConfiguration.Alert.AddExistingOnVariantUnsupported", strLanguage);
      ...
      if(strParentType != null && !"".equals(strParentType))
      {
         if(bInvalidState){
             ...
             <%
        	      ...
        	      StringList varSubTypes = ProductLineUtil.getChildTypesIncludingSelf(context, ConfigurationConstants.TYPE_VARIANT);
        	      if(varSubTypes.contains(strParentType)){
        		  	  strInclusionFeatureTypes = EnoviaResourceBundle.getProperty(context,"emxConfiguration.VariantValue.AddExisting.Features.IncludeTypes");
        	      }else{
        	    	  strInclusionFeatureTypes = EnoviaResourceBundle.getProperty(context,"emxConfiguration.Variant.AddExisting.Features.IncludeTypes");
        	      }
              ...
              %>
              <body>   
              <form name="FTRVariantFullSearch" method="post">
              <script language="Javascript">
                  ...
                  var submitURL = "../common/emxFullSearch.jsp?objectId=<%=XSSUtil.encodeForURL(context,strObjectID)%>&txtType=<%=XSSUtil.encodeForURL(context,strParentType)%>&field=TYPES="+strFeatureTypes+":CURRENT!=policy_ConfigurationFeature.state_Obsolete&excludeOIDprogram=ConfigurationFeature:excludeAvailableVariants&table=FTRConfigurationFeaturesSearchResultsTable&HelpMarker=emxhelpfullsearch&selection=multiple&showInitialResults=false&showSavedQuery=true&hideHeader=true&formInclusionList=<%=XSSUtil.encodeForURL(context,strformInclusionList)%>&context="+context+"&suiteKey=Configuration&submitURL=../configuration/VariantsAddExistingPostProcess.jsp?suiteKey=Configuration&StringResourceFileId=emxConfigurationStringResource&SuiteDirectory=configuration&objectId=<%=XSSUtil.encodeForURL(context,strObjectID)%>&parentOID=<%=XSSUtil.encodeForURL(context,parentId)%>&emxExpandFilter=1&isModel=<%=XSSUtil.encodeForURL(context,isModel)%>&parentForIRule=<%=XSSUtil.encodeForURL(context,parentForIRule)%>";

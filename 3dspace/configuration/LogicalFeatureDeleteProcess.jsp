...
if(null!=strMode && strMode.equalsIgnoreCase("deleteLF")){
	try{
        ...
        if(OIDsToDelete!=null){
	        ...
	        LogicalFeature.delete(context, sLOIDsToDelete);              
    ...
    if(arrTableRowIds[0].endsWith("|0")){
 	   ...
 	   if(unsavedData == true){//Markup selected case
	    	...
	    	for(int i=0;i<arrTableRowIds.length;i++){
	            ...
	            <%
		    }//populate js arrray and java object list 
	         ...
	         <%
		         ...
		         %>
		            ...
		            if(contentFrameObj.isDataModified()){//if SB is Modified- will not allow to delete object
			            ...
			            var urlParams = "mode=deleteLF&OIDsToDelete="+RemMPRelIDs;
			            var vRes = emxUICore.getDataPost("../configuration/LogicalFeatureDeleteProcess.jsp", urlParams);
		                ...
		                if("removeandrefresh"===action){//if Delete action is OK remove selected node from UX
		                   ...
		                   for(var i = 0; i < cBoxArray.length; i++){	  
		      	           ...
		      	           if("FTRContextLFLogicalFeatures"===VarPortalCmdName ||"FTRSystemArchitectureLogicalFeatures"===VarPortalCmdName ){//only incase of Product Context Tree need to refreshed to remove selected Objects
    	...
    	}//selected node is not markup
  	}//non root node selected
}//mode not passed
   %>

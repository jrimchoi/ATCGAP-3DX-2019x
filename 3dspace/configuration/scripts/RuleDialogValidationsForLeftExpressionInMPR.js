...
function insertFeatureOptions(textArea,parentOID)
{
    ...
    for(var key in vchkItems)
    {
        ...
        // To Forbid selection of Products in rules creation/edition panel- removed isRootNodeProduct check 
    	...
    	{
    	    var vUrlParam = "mode=formatMPR&checkedRelIds="+checkedRelIds+"&checkedParentIds="+checkedParentIds+"&checkedObjectIds="+checkedObjectIds+"&featureOptionDetails="+featureOptionDetails;
        	...
        	if(textArea == 'right')
        	{
        		if(checkedParentIds.trim().length!=0 && (isOfTypeFeatures=='Yes' ||(isOfTypeFeatures=='No'&& vProductId!=null && vProductId!=checkedParentIds)))
        		{

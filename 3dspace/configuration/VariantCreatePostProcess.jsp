...
<%
  try
  {
      ...
      // Create Variant Object from Global Actions.
      if("GlobalActions".equalsIgnoreCase(strUIContext))
      {
          ...
          %>
             <script language="javascript" type="text/javaScript">
                ...
                contentFrameObj.document.location.href= "../common/emxTree.jsp?objectId=<%=XSSUtil.encodeForURL(context,strNewObjId)%>&jsTreeID=<%=XSSUtil.encodeForURL(context,strJsTreeID)%>&treeMenu=type_Variant";             
             </script>
          <%
      }
      else // Create Variant Object from Product Line/Model/Model Version(Product) Context.
      {
    	  // To get the relationship id between newly created Variant Value and Variant under which it is created.
          matrix.db.Context ctx = (matrix.db.Context)request.getAttribute("context");
    	  ContextUtil.commitTransaction(ctx);
    	  ...
    	  MapList mapModelStructure = domObj.getRelatedObjects(context, ConfigurationConstants.RELATIONSHIP_CONFIGURATION_FEATURES, ConfigurationConstants.TYPE_VARIANT, null, new StringList(DomainRelationship.SELECT_ID),false,true,(short)1,strWhereExp,DomainConstants.EMPTY_STRING,1);

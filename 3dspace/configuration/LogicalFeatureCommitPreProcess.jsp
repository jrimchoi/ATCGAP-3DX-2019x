<%--
  LogicalFeatureCommitPreProcess.jsp
  ...
  {     
      ...
      if(isCandidateFeatureSelected == false){        
         ...
         <%
    	  ...
    	  session.setAttribute("selectedCandidateFeatures",strObjectIdList);
     %>
     <body>   
     <form name="FTRModelConnectedProductSelection" method="post">
     <script language="Javascript">
         var submitURL = "../common/emxIndentedTable.jsp?program=LogicalFeature:getRelatedProductsForContext&expandProgram=LogicalFeature:getTempCommittedLogicalFeatures&table=FTRModelCommitToProductTable&selection=multiple&header=emxConfiguration.CommitCandidate.LogicalFeatures&toolbar=FTRModelLogicalFeatureCommitToToolbar&objectCompare=false&mode=edit&editLink=<%=XSSUtil.encodeForURL(context,editLink)%>&suiteKey=Configuration&StringResourceFileId=emxConfigurationStringResource&SuiteDirectory=configuration&massUpdate=false&massPromoteDemote=false&parentOID=<%=XSSUtil.encodeForURL(context,strObjIdContext)%>&objectId=<%=XSSUtil.encodeForURL(context,strObjIdContext)%>&connectionProgram=LogicalFeature:commitCandidateFeatureToProduct&postProcessJPO=LogicalFeature:refreshCandidateFeaturePageOnApply&context=LogicalFeature&HelpMarker=emxhelplogicalfeaturecommit";
         showModalDialog(submitURL,850,630,"true","Large");
     </script>
     </form>
     </body>
     <% 

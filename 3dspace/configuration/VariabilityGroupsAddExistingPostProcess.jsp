<%--
  VariantsAddExistingPostProcess.jsp
...
<%
  ...
  {  
     ...
     String strContextNotSupportedForVariabilityGroup = EnoviaResourceBundle.getProperty(context,"Configuration","emxConfiguration.Feature.Create.ContextNotSupportedForVariabilityGroup",strLanguage);
    ...
    //If the selection are not made in Search results page  
     if(strArrSelectedTableRowId==null){
	     ...
	     <%
	         ...
	         StringList cfSubTypes = ProductLineUtil.getChildTypesIncludingSelf(context, ConfigurationConstants.TYPE_VARIABILITYGROUP);
	         ...
	         StringList coSubTypes = ProductLineUtil.getChildTypesIncludingSelf(context, ConfigurationConstants.TYPE_VARIABILITYOPTION);
             ...
             for(int x=0;x<strArrSelectedTableRowId.length;x++){
	             ...
	             if( !cfSubTypes.contains(strParentType) &&  !plSubTypes.contains(strParentType) && !prdSubTypes.contains(strParentType)&& !modSubTypes.contains(strParentType))
	             {
	            	 %>
	                 <script language="javascript" type="text/javaScript">
	                       alert("<%=XSSUtil.encodeForJavaScript(context,strContextNotSupportedForVariabilityGroup)%>");
	                 </script>
		            <%
	        	...
	        	// checking for different RDO alert
                 ...
                 while(itr.hasNext()){
                     strSelectedObject = itr.next().toString();
                         ...
                         confModel.connectCandidateConfigurationFeature(context, strSelectedObject);

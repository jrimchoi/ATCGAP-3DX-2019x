...
<%
  ...
  {	  
	  ...
	  String strRemoveOnVariantUnsupported = EnoviaResourceBundle.getProperty(context,"Configuration","emxConfiguration.Alert.RemoveOnVariantUnsupported", strLanguage);
      ...
      if(arrTableRowIds!=null && arrTableRowIds[0].endsWith("|0")){
	   ...
	   if(unsavedData == true){
          ...
          boolean isSelectedCOType = false;//ConfigurationUtil.isListContainsTypeOfKind(context, strObjectIdList,ConfigurationConstants.TYPE_VARIANTVALUE);
          if(isSelectedCOType){
              %>
              <script language="javascript" type="text/javaScript">
                    alert("<%=XSSUtil.encodeForJavaScript(context,strRemoveOnVariantUnsupported)%>");
              </script>
             <%
          }else{

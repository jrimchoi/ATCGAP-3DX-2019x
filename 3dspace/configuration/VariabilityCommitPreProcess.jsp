<%--
  VariabilityCommitPreProcess.jsp
...
if(isMobileMode){
  ...
  {	        
      ...
      String strOnlyCandidateToCommit = EnoviaResourceBundle.getProperty(context,"Configuration","emxProduct.Alert.OnlyCandidateToCommit", strLanguage);
	  ...
	  if(mapList.size() > 0)
	  {
	   ...
	   if(isCommittedOrMandatoryRel)
	   {
          ...
          <script language="javascript" type="text/javaScript">
                alert("<%=strOnlyCandidateToCommit%>");
          </script>
          <%
	    }
	    else if(isCandidateFeatureSelected == false)
	    {    	  
    	  %>
          <script language="javascript" type="text/javaScript">
        ...
        { 
	      ...
	      <script language="Javascript">     
	          var submitURL = "../common/emxIndentedTable.jsp?program=ConfigurationFeature:getRelatedProductsForContext&expandProgram=ConfigurationFeature:getTempCommittedConfigurationFeatures&table=FTRModelCommitToProductTable&selection=multiple&header=emxProduct.GBOMStructureBrowser.ViewGBOMPartTableHeader&toolbar=FTRModelConfigurationFeatureCommitToToolbar&objectCompare=false&mode=edit&editLink=<%=XSSUtil.encodeForURL(context,editLink)%>&suiteKey=Configuration&StringResourceFileId=emxConfigurationStringResource&SuiteDirectory=configuration&massPromoteDemote=false&parentOID=<%=XSSUtil.encodeForURL(context,strObjIdContext)%>&objectId=<%=XSSUtil.encodeForURL(context,strObjIdContext)%>&connectionProgram=ConfigurationFeature:commitCandidateFeatureToProduct&postProcessJPO=ConfigurationFeature:refreshCandidateFeaturePageOnApply&context=ConfigurationFeature&HelpMarker=emxhelpvariantcommit";

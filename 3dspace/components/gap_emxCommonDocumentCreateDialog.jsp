...
<!-- ENGMASA : Modified textbox to dispaly as readonly and hide autoname to display always auto number : START -->
<%!
    protected String generateDocumentName(Context context) throws Exception{
    	...
    	MapList agObjectGenerators = DomainObject.findObjects (context,
									                ...
									                "type_DOCUMENTS", // name pattern e.g. value Test Everything,dmc author
									                "*",
									                "*",
    	...
    	if(agObjectGenerators.size() != 1){
    		throw new Exception("Error in Generating Document Name");
    	}
...
%>
<!-- ENGMASA : Modified textbox to dispaly as readonly and hide autoname to display always auto number : END -->
...
public String getBookmarkName(Context context, String sObjectId) throws Exception {
	...
	if(UIUtil.isNullOrEmpty(sBookmarkName)) {
		...
		getBookmarkName(context, sBookmarkVaultId);
	}
...
<%
  ...
  {
      // <!-- ENGMASA : Modified textbox to dispaly as readonly and hide autoname to display always auto number : START -->
	  documentName = generateDocumentName(context);
	  if (UIUtil.isNullOrEmpty(documentName))
             documentName = "";
	  // <!-- ENGMASA : Modified textbox to dispaly as readonly and hide autoname to display always auto number : END -->
  }
  ...
  else
  {
    ...
    // Razorleaf fix start
  String strSymbolicChosenType = "";
  if( documentType != null)
  {
    strSymbolicChosenType = FrameworkUtil.getAliasForAdmin(context, "type", documentType, true);
	System.out.println("Just set symbolic type to: " + strSymbolicChosenType);
  }
  // Razorleaf fix end
  ...
  if( documentPolicy == null || "null".equals(documentPolicy) || "".equals(documentPolicy) || bTypeChanged)
  {
      ...
      try
      {
        // Razorleaf fix start - needed to change this to the actual symbolic name of the selected type - it was the generic symbolic name before...
		//documentPolicy = EnoviaResourceBundle.getProperty(context,"emxComponents.DefaultPolicy." + symbolicDocumentType);
		documentPolicy = EnoviaResourceBundle.getProperty(context,"emxComponents.DefaultPolicy." + strSymbolicChosenType);
		// Razorleaf fix end
...
%>
  ...
  // Razorleaf Doc Family Chooser start
  function showDocFamilySelector()
  {
    //document.frmMain.typeChanged.value="true";
    var strURL="../common/emxFullSearch.jsp?field=TYPES=type_DocumentFamily&showSavedQuery=false&table=LCClassificationList&mode=Chooser&selection=single&chooserType=DocFamilyChooser&typeAheadTable=LBCClassifyTable&fieldNameActual=realDocFamily&fieldNameDisplay=docFamily&fieldNameOID=realDocFamilyOID&submitURL=../components/emxComponentsFullSearchUtil.jsp";
    var win = showModalDialog(strURL, 700, 600);
  }  
  // Razorleaf Doc Family Chooser end
  ...
  function reload() {
	  ...
	  document.frmMain.action="../components/emxCommonDocumentCreateDialog.jsp?reloadPage=true&contentPageIsDialog=true&parentId=<%=objectId%>";
	  // TBIAPRA : Added below methods for Issue Number 70: Enhancement: Enable Job number field on GAP Specification creation under Part : END
      document.frmMain.submit();
  }
...
// ENGMASA : Added below code to configure document code field : START
	function showDocumentCodeSelector() {
		var vSelectedVal = document.frmMain.gapDocumentCode.value;
		//alert("gapDocumentCode : "+vSelectedVal);
		 var strURL="../common/gapRangeAttributeChooser.jsp?attributeName=attribute_gapDocumentCode&attrDispName=Document Code&formName=frmMain&fieldActual=gapDocumentCode&attributeVal="+vSelectedVal;
		var win = showModalDialog(strURL, 450, 500, true);		
	}
// ENGMASA : Added below code to configure document code field : END
...
%>
        ...
        <input type="checkbox" name="AutoName" style="display:none" value="<xss:encodeForHTMLAttribute><%=documentAutoName%></xss:encodeForHTMLAttribute>" onClick="txtNameFocus()"/>&nbsp;
        <!-- <emxUtil:i18n localize="i18nId">emxComponents.Common.AutoName</emxUtil:i18n> -->
		<!-- ENGMASA : Modified textbox to dispaly as readonly and hide autoname to display always auto number : END -->
<%
      }
...
<%
      ...
      else
      {
		...
		// TBIAPRA : Added below methods for Issue Number 70: Enhancement: Enable Job number field on GAP Specification creation under Part : START
		if(attributeName.equals("gapProjectNumber")) {
			attributeValue = (String) getBookmarkName(context, objectId);
		}
		// TBIAPRA : Added below methods for Issue Number 70: Enhancement: Enable Job number field on GAP Specification creation under Part : END
		...
		// TBIAPRA : Added below methods for Issue Number 70: Enhancement: Enable Job number field on GAP Specification creation under Part : START
		if (symbolicAttributeName.equals("attribute_gapProjectNumber")) {
				%>
				<tr>
				  <td class="label" >
					<%= XSSUtil.encodeForHTML(context, i18nNow.getAttributeI18NString(attributeName,sLanguage))%>
				  ...
				  <td class="inputField">
				  <input type="text" name="<xss:encodeForHTMLAttribute><%=attributeName%></xss:encodeForHTMLAttribute>"
					  value="<xss:encodeForHTMLAttribute><%=attributeValue%></xss:encodeForHTMLAttribute>"  readonly /></td>
			    </tr>
				<%
		}
		// TBIAPRA : Added below methods for Issue Number 70: Enhancement: Enable Job number field on GAP Specification creation under Part : END
		...
		// ENGMASA : Added below code to configure document code field : START
		else if (symbolicAttributeName.equals("attribute_gapDocumentCode"))
		{
				%>
				<tr>
				  <td class="label" >
				  ...
				  <td class="inputField">
				  <input type="text" name="<xss:encodeForHTMLAttribute><%=attributeName%></xss:encodeForHTMLAttribute>"
					  value="<xss:encodeForHTMLAttribute><%=attributeValue%></xss:encodeForHTMLAttribute>"  readonly />
				   <input type="button" name="gapDocCodeChooser" value=".." size="5" onClick="showDocumentCodeSelector();"/></td>
			    </tr>
				<%
		}else{
		// ENGMASA : Added below code to configure document code field : END
...
<% } %>
	...
	<!-- Razorleaf "Document Family" input start-->
    ...
    <tr><!--//XSSOK-->
      ...
      <td class = "inputField" >
          <!-- //XSSOK -->
          <input type="text" name="docFamily" id="docFamily" value="" size="20" readonly />
          <input type="button" name="FieldButton" value=".." size="5" onClick="showDocFamilySelector()"/>
          <!-- //XSSOK -->
          <input type="hidden" name="realDocFamily" id="realDocFamily" value="" />
		  <input type="hidden" name="realDocFamilyOID" id="realDocFamilyOID" value=""/>
      </td>
    </tr>
	
	<!-- Razorleaf "Document Family" input end-->

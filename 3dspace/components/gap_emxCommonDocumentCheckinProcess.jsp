...
<%
  ...
  try
  {
      ...
      Enumeration enumParam;
		...
		// Razorleaf - connect new doc to the Document Family, if there is one start
		Object oObjectIds = (Object) objectMap.get("objectId");
		String newDocId = "";
		if (oObjectIds.getClass() == StringList.class)
		{
			StringList slObjectIds = (StringList) oObjectIds;
			System.out.println("Object IDs: " + slObjectIds.toString());
			newDocId = (String) slObjectIds.elementAt(0);
		} else if (oObjectIds.getClass() == String.class) {
			newDocId = (String) oObjectIds;
		}
		
		String docFamilyId = (String)requestMap.get("realDocFamilyOID");
		String sDocFamilyRelType = "Classified Item";
		DomainObject doNewDocument = DomainObject.newInstance(context, newDocId);
		
		//System.out.println("\n\n\n\n\n****************************About to connect new doc to a doc family - Doc OID: " + newDocId + " - Doc Family OID: " + docFamilyId);
		
		if (UIUtil.isNotNullAndNotEmpty(docFamilyId))
		{
			DomainObject doDocumentFamily = DomainObject.newInstance(context, docFamilyId);
			doNewDocument.connect(context, sDocFamilyRelType, doDocumentFamily, true);
		} 
		// Razorleaf - connect new doc to the Document Family, if there is one end

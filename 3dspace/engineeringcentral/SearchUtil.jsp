...
if(UIUtil.isNotNullAndNotEmpty(addMasterReference)){
  	...
  	HashMap <String,String> addReferenceToMasterInfo   = new HashMap<String,String>();
  	Map map = new HashMap();
  	map.put("objectId",strObjId);
  	map.put("MasterId",MasterId);
  	map.put("emxTableRowId",selectedItems);
  	String [] args = JPO.packArgs(map);
  	addReferenceToMasterInfo = (HashMap)JPO.invoke(context,"emxPartFamily",null,"addReferencestoMaster",args,HashMap.class);
	String alertMsg = addReferenceToMasterInfo.get("displayMessage");
...
%>
						 ...
						 var vfieldRDODisplay = targetWindow.document.getElementsByName("<xss:encodeForJavaScript><%= fieldNameRDODisplay%></xss:encodeForJavaScript>");
	    	 ...
	    	 %>
		        ...
		        if (strFromSide.equals("false")){
			        ...
			        for(int i=0;i<strContextObjectId.length;i++){
					  ...
					  if(DomainConstants.RELATIONSHIP_REFERENCE_DOCUMENT.equals(strRelName))
					{
					...
					var sSumittedFrom = "<xss:encodeForJavaScript><%=sSumittedFrom%></xss:encodeForJavaScript>";
					...
					if(sSumittedFrom != null && sSumittedFrom != "undefined" && sSumittedFrom == "AddExistingDocument"){
							if(typeof getTopWindow().SnN != 'undefined' && getTopWindow().SnN.FROM_SNN){
								findFrame(getTopWindow(),"<%=frameName%>").refreshSBTable();
							}
							else
		...
		//Modified for To Create Multiple part from Part Clone
			...
			var tempNameField = targetWindow.document.forms[0]["nameField"].value;
			 /* if(getTopWindow().opener) {
				getTopWindow().opener.turnOnProgress();
			}	 */ 
			if(getTopWindow().getWindowOpener()) {
				getTopWindow().getWindowOpener().turnOnProgress();
			}
			//XSSOK
			targetWindow.location.href = "../common/emxCreate.jsp?submitAction=doNothing&nameField=" + tempNameField + "&typeChooser=true&header=emxEngineeringCentral.Part.ClonePart&form=ENCClonePart&suiteKey=EngineeringCentral&multiPartCreation=true&createMode=ENG&HelpMarker=emxhelppartclone&copyObjectId=<%=XSSUtil.encodeForJavaScript(context,strObjectId)%>&type=<%=selectedObjectType%>&postProcessJPO=emxPart:postProcessForClonePart&preProcessJavaScript=preProcessInCreatePartClone&TypeActual=Part&createJPO=emxPart:checkLicenseAndCloneObject&partFamilyID=<%=pfId%>&PartFamilyAutoName=<%=pfAutoNameGenerator%>&postProcessURL=../engineeringcentral/PartCreatePostProcess.jsp?mode=clonedPartOpenInEditMode&emxTableRowId=<%=XSSUtil.encodeForJavaScript(context,strObjectId)%>&targetLocation=slidein&slideinType=wider";
			if(typeAhead != "true" && !(typeof getTopWindow().SnN != 'undefined')){
				getTopWindow().closeWindow();
			}
		</script>	         
<%
...
%>
                 <script language="javascript" type="text/javaScript">
	      			...
	      			if ( !( typeof getTopWindow().SnN != 'undefined' && getTopWindow().SnN.FROM_SNN) ) {
                        ...
                        findFrame(getTopWindow().getWindowOpener().getTopWindow(), "ENCBOM").setRequestSetting("workUnderChangeId", "<xss:encodeForJavaScript><%=strObjectId%></xss:encodeForJavaScript>");
	      			}
	      			else{
					  vfieldNameActual = findFrame(getTopWindow(), "ENCBOM").document.getElementById("<%=fieldNameActual%>");
					  vfieldNameDisplay = findFrame(getTopWindow(), "ENCBOM").document.getElementById("<%=fieldNameDisplay%>");
					  
					  findFrame(getTopWindow(), "ENCBOM").setRequestSetting("workUnderChangeId", "<xss:encodeForJavaScript><%=strObjectId%></xss:encodeForJavaScript>");				  
				  }
	      			//XSSOK
	      		  vfieldNameDisplay.value ="<%=strContextObjectName%>" ;
               //XSSOK

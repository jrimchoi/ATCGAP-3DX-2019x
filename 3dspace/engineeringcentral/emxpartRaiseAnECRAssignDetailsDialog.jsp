<%--  emxpartRaiseAnECRAssignDetailsDialog.jsp   -
...
<%
  ECR ecrId = (ECR)DomainObject.newInstance(context,DomainConstants.TYPE_ECR,DomainConstants.ENGINEERING);
...
String rdoName      = (String)session.getAttribute("rdoName");
  ...
  //boolean blnIsDynamic = true;
	...
	DomainObject doECR = new DomainObject(selectedType);

	String strPolicy = doECR.getInfo(context, DomainConstants.SELECT_POLICY);
	...
	if (Relationship == null || Relationship.indexOf("null") != -1)
	{
		...
		//else
		//{
		//	Relationship = DomainConstants.RELATIONSHIP_REQUEST_PART_REVISION;
		//}
	}
  ...
  if(Create==null || "null".equals(Create)){
   ...
   if(Create.equals("true")){
    backActionUrl="emxpartCreateRaiseECRWizardFS.jsp?prevmode=true&Relationship="+Relationship;
   }
   //Added for   IR-068172V6R2012 starts
   else if(null!=mode&&"ExistingECR".equals(mode))
   {  
	 ...
	 String someString =EnoviaResourceBundle.getProperty(context, "emxEngineeringCentralStringResource", context.getLocale(),"emxEngineeringCentral.RaiseECO.SearchAnECO");
	    //Modified for IR-172253V6R2013x 
	   //backActionUrl=  "../common/emxFullSearch.jsp?field=TYPES=type_ECR&HelpMarker=emxhelpfullsearch&table=ENCGeneralSearchResult&selection=single&submitURL=../engineeringcentral/emxEngrRaiseECRECOHiddenProcess.jsp&calledMethod=RaiseECR&jsTreeID=" + jsTreeID + "&objectId=" + objectId + "&suiteKey=" + suiteKey + "&Relationship="+Relationship+"&header="+ someString +"&hideHeader =false";
	     backActionUrl=  "../common/emxFullSearch.jsp?field=TYPES=type_ECR:CURRENT=policy_ECR.state_Create,policy_ECR.state_Submit,policy_ECR.state_Evaluate&HelpMarker=emxhelpfullsearch&table=ENCGeneralSearchResult&selection=single&submitURL=../engineeringcentral/emxEngrRaiseECRECOHiddenProcess.jsp&calledMethod=RaiseECR&jsTreeID=" + jsTreeID + "&objectId=" + objectId + "&suiteKey=" + suiteKey + "&Relationship="+Relationship+"&header="+ someString +"&hideHeader =false";
    
    }
   //Added for   IR-068172V6R2012 ends
   else
   {
     //backActionUrl="emxpartRaiseECRSearchResultsFS.jsp?prevmode=true&Relationship="+Relationship;
     backActionUrl="emxpartRaiseECRSearchResultsFS.jsp?prevmode=true";//IR:069928
...
sVault = "eService Production";
  ...
  if(!(Create.equals("true"))) {
    ...
    //String relPattern =   DomainConstants.RELATIONSHIP_REQUEST_PART_OBSOLESCENCE + ","+   DomainConstants.RELATIONSHIP_REQUEST_PART_REVISION + ","+   DomainConstants.RELATIONSHIP_REQUEST_SPECIFICATION_REVISION;
...
%>
      <table border="0" cellpadding="5" cellspacing="2" width="100%">
        <tr>
          ...
          <td class="field"><emxUtil:i18n localize="i18nId">emxEngineeringCentral.RaiseECR.WarningMessage</emxUtil:i18n></td>
        </tr>
        <tr>
          <td width="25%" class="label">&nbsp;</td>
          <td class="field"><a href="JavaScript:ECRRaisedWindow()"><emxUtil:i18n localize="i18nId">emxEngineeringCentral.RaiseECR.ItemsExistingECRHref</emxUtil:i18n></a></td>
        </tr>
      </table>
<%
  }
...
%>
	<!-- XSSOK -->
          <textarea name="<%=attrName%>" style="font-size: 8pt; color:black" rows="3" cols="40"  wrap="soft"><%=mapinfo.get("value")%></textarea>
<%
...
<script language="Javascript">
  ...
  function submitForm() {
	document.relattrFrm.target="_parent";
	document.relattrFrm.action="emxpartReasonForChangeAnECRWizardFS.jsp";
    document.relattrFrm.submit();
    return;
  }
  ...
  function ECRRaisedWindow() {
	 ...
	 //XSSOK
	 document.relattrFrm.action='<%=URLtoECRRaisedPage%>';
     document.relattrFrm.submit();
     newWin1.focus();
     return;

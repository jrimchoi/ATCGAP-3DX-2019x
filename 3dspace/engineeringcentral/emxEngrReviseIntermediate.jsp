...
String strApplyBGTPItemMarkup   = EnoviaResourceBundle.getProperty(context, "emxEngineeringCentralStringResource", 
	...
	/* block the Revise operation if the revise source object is Strict Mode. */
	boolean allowReviseAction = true;
	String manualReviseBLockErrorMessage = EnoviaResourceBundle.getProperty(context, "emxEngineeringCentralStringResource", context.getLocale(), "emxEngineeringCentral.Alert.ManualReviseBlock"); 
	if ( "Revise".equals(strAction) ) {
        if ( DomainObject.newInstance(context, objectId).isKindOf(context, DomainConstants.TYPE_PART) ) {
            String latestId = DomainObject.newInstance(context, objectId).getInfo(context, "last.id");
            IPartService iPartService = PartMgtFactory.getPartService(context, latestId);
            if ( !iPartService.getInfo(context).isReleased() && !"Complete".equalsIgnoreCase(iPartService.getInfo(context).getCurrent()) ) {
                allowReviseAction = !iPartService.isChangeControlled(context);
            }
        }
	}
    /* block the Revise operation if the revise source object is Strict Mode. */
	...
	if(UIUtil.isNotNullAndNotEmpty(lastRevObjId)){
		programMap.put("objectId",lastRevObjId);
    ...
    if(dmObj.isKindOf(context, DomainObject.TYPE_PART)) {
        ...
        postProcessURL = "../engineeringcentral/emxCreatePartRevisionPostProcess.jsp";
        helpMarker = "emxhelppartrevisions";
    }
	boolean isPart=dmObj.isKindOf(context, DomainObject.TYPE_PART);
%>
...
if ("<%=XSSUtil.encodeForJavaScript(context,showMode)%>" == "slideIn") { 
	//XSSOK
	//Modified for Planning MBOM-Planning Required-Start
    var sURL = "../common/emxCreate.jsp?lastRevPolicy=<xss:encodeForURL><%=sPolicy%></xss:encodeForURL>&lastRevVault=<xss:encodeForURL><%=lastRevVault%></xss:encodeForURL>&copyObjectId=<xss:encodeForURL><%=lastRevObjId%></xss:encodeForURL>&latestRevision=<xss:encodeForURL><%=latestRevision%></xss:encodeForURL>&highNum=<xss:encodeForURL><%=highNum%></xss:encodeForURL>&highStr=<xss:encodeForURL><%=highStr%></xss:encodeForURL>&form=<xss:encodeForURL><%=formName%></xss:encodeForURL>&header=<xss:encodeForURL><%=headerStr%></xss:encodeForURL>&type=<xss:encodeForURL><%=symbolicName%></xss:encodeForURL>&suiteKey=EngineeringCentral&StringResourceFileId=emxEngineeringCentralStringResource&SuiteDirectory=engineeringcentral&submitAction=refreshCaller&createJPO=<xss:encodeForURL><%=createJPO%></xss:encodeForURL>&preProcessJavaScript=<xss:encodeForURL><%=preProcessJS%></xss:encodeForURL>&HelpMarker=<xss:encodeForURL><%=helpMarker%></xss:encodeForURL>&hasRDO=<xss:encodeForURL><%=hasRDO%></xss:encodeForURL>&suiteKey=EngineeringCentral&StringResourceFileId=emxEngineeringCentralStringResource&SuiteDirectory=engineeringcentral&partName=<xss:encodeForURL><%=sName%></xss:encodeForURL>&nextRev=<xss:encodeForURL><%=nextRev%></xss:encodeForURL>&reviseAction=true&createMode=ENG&postProcessURL=<xss:encodeForURL><%=postProcessURL%></xss:encodeForURL>&prevUOM=<xss:encodeForURL><%=prevUOM%></xss:encodeForURL>";
    if("<%=isPart%>"=="true"){
        sURL = "../common/emxCreate.jsp?lastRevPolicy=<xss:encodeForURL><%=sPolicy%></xss:encodeForURL>&lastRevVault=<xss:encodeForURL><%=lastRevVault%></xss:encodeForURL>&copyObjectId=<xss:encodeForURL><%=lastRevObjId%></xss:encodeForURL>&latestRevision=<xss:encodeForURL><%=latestRevision%></xss:encodeForURL>&highNum=<xss:encodeForURL><%=highNum%></xss:encodeForURL>&highStr=<xss:encodeForURL><%=highStr%></xss:encodeForURL>&form=<xss:encodeForURL><%=formName%></xss:encodeForURL>&header=<xss:encodeForURL><%=headerStr%></xss:encodeForURL>&type=<xss:encodeForURL><%=symbolicName%></xss:encodeForURL>&suiteKey=EngineeringCentral&StringResourceFileId=emxEngineeringCentralStringResource&SuiteDirectory=engineeringcentral&submitAction=doNothing&createJPO=<xss:encodeForURL><%=createJPO%></xss:encodeForURL>&preProcessJavaScript=<xss:encodeForURL><%=preProcessJS%></xss:encodeForURL>&HelpMarker=<xss:encodeForURL><%=helpMarker%></xss:encodeForURL>&hasRDO=<xss:encodeForURL><%=hasRDO%></xss:encodeForURL>&suiteKey=EngineeringCentral&StringResourceFileId=emxEngineeringCentralStringResource&SuiteDirectory=engineeringcentral&partName=<xss:encodeForURL><%=sName%></xss:encodeForURL>&nextRev=<xss:encodeForURL><%=nextRev%></xss:encodeForURL>&changeControlled=<xss:encodeForURL><%=changeControlledValue%></xss:encodeForURL>&reviseAction=true&createMode=ENG&postProcessURL=<xss:encodeForURL><%=postProcessURL%></xss:encodeForURL>&prevUOM=<xss:encodeForURL><%=prevUOM%></xss:encodeForURL>";
     	 if("<xss:encodeForJavaScript><%=portalCmdName%></xss:encodeForJavaScript>")
     		sURL=sURL+"&portalCmdName=<xss:encodeForURL><%=portalCmdName%></xss:encodeForURL>";
     	 if("<xss:encodeForJavaScript><%=frameName%></xss:encodeForJavaScript>")
     		sURL=sURL+"&frameName=<xss:encodeForURL><%=frameName%></xss:encodeForURL>";     	
     }
    ...
    if("<%=isPart%>"=="true") {
    	if ("<%= allowReviseAction %>" == "true") {
    		   getTopWindow().showSlideInDialog(sURL, true, null, null, 550);
    	} else {
    		//XSSOK
    		alert ("<%= manualReviseBLockErrorMessage %>");
    	}
    }
    else

<%--  emxEngrECRPdfSummaryReport.jsp   -  This page displays the ECR Summary Report
...
<%
   ...
   // Generate summary if parameter 'generateSummary' is true
	   ...
	   try {
                    IsReportGenerationSuccess = cxBean.createECRPDFSummaryReport(
                            context, methodargs);
	   }catch(Exception e) {
	   ...
	   if(IsReportGenerationSuccess == 1) {
                    errorMessage
                            .append(EngineeringUtil.i18nStringNow(context,
   ...
   //Set the objectId in session
   session.setAttribute("ECRSummaryReportObjectId",objectId);
   session.setAttribute("ECRSummaryReportCategoryTreeMenu", strCategoryTreeName);

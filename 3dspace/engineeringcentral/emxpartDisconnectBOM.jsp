...
<%
  boolean isMBOMInstalled = com.matrixone.apps.engineering.EngineeringUtil.isMBOMInstalled(context);
    ...
    String QuantityForRel = DomainConstants.EMPTY_STRING;
    String selPartRowId = "";
	...
	if(isMBOMInstalled && "common".equalsIgnoreCase(sBOMFilterVal)){
		...
		for (int i = 0; i < sCheckBoxArray.length; i++) {
			...
			if(!strSelectedPartId.startsWith("|")){
				...
				if(!"0".equalsIgnoreCase((String) slRowInfo.get(slRowInfo.size()-1)) && UIUtil.isNotNullAndNotEmpty(strSelectedRelId)){
				   ...
				   if(DomainConstants.RELATIONSHIP_EBOM.equalsIgnoreCase(sRelType)) {	
					   strErrMsg = EnoviaResourceBundle.getProperty(context,"emxMBOMStringResource", context.getLocale(),"emxMBOM.EBOM.EBOMRemoveNotAllowedFromCommonView");		
			 	      break;
				   }
		...
		StringList selectedRowInfo = StringUtil.split(selectedRows, "~");
		for (int i = 0; i < selectedRowInfo.size(); i++) {
			String strSelectedPartId = (String) selectedRowInfo.get(i);
			if(!strSelectedPartId.startsWith("|")){
				StringList slRowInfo = FrameworkUtil.split(strSelectedPartId, "|");
				String strSelectedRelId = (String) slRowInfo.get(0);
				QuantityForRel = BOMMgtUtil.getRollupQuantity(context, strSelectedRelId);
				rowlevelData = rowlevelData +"\""+ strSelectedRelId+"\":\""+ QuantityForRel+"\",";
			}
		}		
	}
   ...
   /* String sConformAsStored      = i18nNow.getI18nString("emxEngineeringCentral.Common.CannotRemoveConnectedParts4",
    ...
    String inlineErrorMessage       = i18nNow.getI18nString("emxFramework.FreezePane.SBEditActions.RowSelectError",
           ...
           String sInCorrectRolledupView      = EnoviaResourceBundle.getProperty(context ,"emxEngineeringCentralStringResource",
                                            context.getLocale(),"emxEngineeringCentral.validate.RefreshRollupView");
...
//Modified for IR042348V6R2011
        ...
        function doInitialProcess(){
            try{
                ...
                var BOMViewMode = window.parent.getParameter("BOMViewMode");
               ...
               // var xCheckedPath  = "/mxRoot/rows//r[@checked='checked']";
                ...
                for(var raa=0; raa<checkedRows.length;raa++){
                    ...
                    if(BOMViewMode == "Rollup"){
	                  //XSSOK
	                    var relRolledUpQtyVal = <%=rowlevelData%>;
	                    var sRollupQty = relRolledUpQtyVal[relid];
	                    var objColumnQty  = dupcolMap.getColumnByName("Quantity");
	                    var sRowQty =  dupemxUICore.selectSingleNode(checkedRow, "c["+objColumnQty.index+"]").getAttribute("a");
	                    if(sRollupQty!=sRowQty){
	                    	//XSSOK
	                    	removeMsg = "<%=sInCorrectRolledupView%>";
	                        break;
	                    }
                    }
        ...
        var displayView = contentFrame.parent.displayView
   		if(contentFrame.parent.editableTable && contentFrame.parent.editableTable != null && contentFrame.parent.editableTable != undefined){
   				...
   				WorkUnderOID = contentFrame.parent.getRequestSetting("ChangeAuthoringContextOID");
 		...
 		if(tablemode != "edit") {
 			...
 			for (i = 0; i < vSelectedObjectArray.length; i++) {
 			    ...
 			    if(selectedObjArray.length >3){
 			    	...
 			    	if(BOMViewMode == "Rollup"){
	 			    	...
	 			    	var relRolledUpQtyVal = <%=rowlevelData%>;
	                    var sRollupQty = relRolledUpQtyVal[relId];
	                    var objColumnQty  = contentFrame.parent.colMap.getColumnByName("Quantity");
	                    var sRowQty =  contentFrame.parent.emxUICore.selectSingleNode(contentFrame.parent.oXML.documentElement, "/mxRoot/rows//r[@id='"+selectedRowId+"']"+ "/c["+objColumnQty.index+"]").getAttribute("a");
	                    if(sRollupQty!=sRowQty){
	                    	//XSSOK
	                    	responseTxt = "<%=sInCorrectRolledupView%>";
	                        break;
	                    }
 			    	}
 			...
 			if(responseTxt==""){
	 			var url="../engineeringcentral/enoDisconnectPart.jsp?";  
	  		  	var strData = "selectedRow="+formatedSelectedRow+"&displayView="+displayView+"&BOMViewMode="+BOMViewMode+"&WorkUnderOID="+WorkUnderOID;

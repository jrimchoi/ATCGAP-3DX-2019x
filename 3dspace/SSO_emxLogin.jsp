...
if (Framework.isLoggedIn(request))
{
    ...
    if (SecurityContext != null && !"".equals(SecurityContext) ) {
     ...
     //JPA-
    targetPage = "oslc/SSO_emxSecurityContextSelection.jsp?rToken=" + request.getParameter("rToken") + "&port=" + request.getParameter("port") + "&ticket=" + request.getParameter("ticket")+ "&redirectParam=" + request.getParameter("redirectParam")+ "&mode=" + request.getParameter("mode");
...
%>
    ...
    if ((FrameworkTarget != null) && (!"".equals(FrameworkTarget)))
    {
        if( FrameworkTarget.contains("emxNavigator.jsp") && forwardURL != null )
        {
            ...
            FrameworkTarget = "/" + FrameworkTarget.split("/")[1] + "/" + FrameworkTarget.split("/")[2] + "/SSO_" + FrameworkTarget.split("/")[3] + "?rToken=" + request.getParameter("rToken") + "&port=" + request.getParameter("port") + "&ticket=" + request.getParameter("ticket");
